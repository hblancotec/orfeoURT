...
define('SERVIDOR_DB', 'basesdnpsql2008');
define('USUARIO_DB',  'orfeo');
define('PASSW_DB',    '3(9iI8Ixds3CIGWSP*V1');
define('NOMBRE_DB',   'GdOrfeo');


//Parametros para configuracion servidor de prueba
//define('SERVIDOR_DB', 'atenea');
...
$server->configureWSDL('metodos scan', $t_namespace);
...
// Register the method to expose la consulta de radicados de un usuario de una dependencia. Recibe como parametro el codigo de usuario y el codigo de la dependencia
$server->register('verifica_archivo',				// method name
	 array(	'nroradicado'	=>	'xsd:string' ),
	array('return' => 'tns:datdigitalizador'),	// output parameters
	$t_namespace,						// namespace
	'urn:orfeoconnect#verifica_archivo',			// soapaction
	'rpc',								// style
	'encoded',							// use
	'Metodo permite la verificacion si un radicado tiene imagenes'			// documentation
);
...
function radicados_usuario($username, $inifecha, $finfecha, $criterio ) {
	...
	if ($rs->RecordCount( ) > 0) {
	    ...
	    if (($usuaEstado==1) && ($permRadicado==1)){
	      ...
	      {   case 'U': {
	              	$tiporad = $criterio{1} ; 
	              	if ($tiporad <> 0) $sqlWhere = " AND (r.RADI_NUME_RADI like '%$tiporad') ";
			            $sqlWhere.= "  AND (SUBSTRING(CAST(r.RADI_NUME_RADI AS varchar(15)), 5, 3) = $usuaDependencia) AND 
               ...
               if ($tiporad <> 0) $sqlWhere = " AND (r.RADI_NUME_RADI like '%$tiporad') "; 
					  	 $coddep = substr($criterio, 2); 
							 $sqlWhere.= "  AND (SUBSTRING(CAST(r.RADI_NUME_RADI AS varchar(15)), 5, 3) = $coddep) 
...
function registrar($pathimagen, $nropaginas, $nroradicado, $usudigitalizador, $observacion ) {

	//Verifico que el archivo (imagen) se encuentre en el servidor
	// $rutImagen = "../orfeo/".$pathimagen;
	...
	$recordR["RADI_NUME_HOJA"]	   =  $nropaginas;
     ...
     // LLenamos el arreglo para registrar el evento   
		 ...
		 $recordH["HIST_OBSE"]      = "'(Se digitalizaron ".$nropaginas." Página(s) del documento ) ".$observacion."'" ;
...
// Verica si un radicado tiene imagenes
function verifica_archivo ($nroradicado)
{
    ...
    $db->SetFetchMode(ADODB_FETCH_ASSOC);
	// 
	$query = 'SELECT RADI_PATH FROM radicado WHERE radi_nume_radi =' .$nroradicado;
	$rscant = $db->Execute($query);
	
	if (!empty($rscant->fields["RADI_PATH"])) {
		     return array('error'=> 'OK',
		                  'mensaje'=> 'El radicado si tiene imagen');
    } else     return array('error'=> 'ERR',
					            'mensaje'=> ' El radicado no tiene imagen');
}
