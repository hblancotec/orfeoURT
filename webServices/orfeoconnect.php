...
$server->wsdl->addComplexType(
		...
		array(
			...
			'codoem'	=>	array( 'name' => 'codoem',	'type' => 'xsd:string'),
		)
	);
...
//Definicion del arreglo para el manejo del metodo chkradicado para la devolucion de informacion
$server->wsdl->addComplexType(
	'inforadicado',
	'complexType',
	'struct',
	...
	array(
		...
		'fechallegada' => array('name' => 'fecha', 'type' => 'xsd:string'),
		'fechaoficio' => array('name' => 'fecha', 'type' => 'xsd:string'),
		'fechaultasig' => array('name' => 'fecha', 'type' => 'xsd:string'),
 		'path'=> array('name' => 'path', 'type' => 'xsd:string'),
        'deprad' => array('name' => 'deprad', 'type' => 'xsd:string'), 		
...
### Metodo: Metodo para la recuperacion de radicados para el aplicativo de Control Interno
$server->register( 'chkradicado',
		array(
			'username'	=>	'xsd:string',
			'password'	=>	'xsd:string',
			'nroradicado'		=>	'xsd:string'
		),
	array('return' => 'tns:inforadicado'),	// output parameters
	$t_namespace,							// namespace
	'urn:orfeoconnect#radicacion',			// soapaction
	'rpc',									// style
	'encoded',								// use
	'Recupera la informacion del radicado a ser consultado Los estados de retorno 0=No existe, 1=No disponible para la dependencia 2=Existe y esta disponible. '	// documentation
);
...
function login($username, $password ) {
	...
	//$db->debug = true;
	$db->Connect('172.20.2.95', 'fldoc', 'accion', 'orfeo');
...
// codoem es el codigo secuencia de destinatarios de empresas sgd_oem_empresa
function radicacion($username, $password, $tiporadicado, $dependestino, $usuadestino, $asunto, $desanexo, $radiFechOfic,
					$trdcodigo, $nombredes, $codoem, $docnro, $codimuni, $codidpto, $idpais, $idcontiente,  $direccion, $telefono, $mail, $nombre )
{
	...
	if ($rs->RecordCount( ) > 0) {
		...
		if ($trdcodigo == 1)	$SecName = "SECR_TP1_100";
		if ($trdcodigo == 2)	$SecName = "SECR_TP2_900";
		if ($trdcodigo == 3)	$SecName = "SECR_TP3_";
    ...
    $recordR['DPTO_CODI']		       =  $codidpto;		
	   ...
	   if ($nextval==-1) return array('Nroradicado'=>'No se ingreso direccion radicado');	
       ...
       $recordD['SGD_DOC_FUN']   = 0;
	   if ($tiporadicado == 3)   $recordD['SGD_OEM_CODIGO'] = $codoem;
	   if ($tiporadicado == 4)   $recordD['SGD_DOC_FUN']    = $docnro;
       $recordD['SGD_DIR_NOMREMDES']  = "'"."$nombredes"."'";
       $recordD['SGD_TRD_CODIGO']     = $tiporadicado;
	...
	if ($rs->RecordCount( ) > 0) {
		...
		{	case '1':
			{   if (strlen($criterio)) $sqlWhere.=" (SGD_CIU_NOMBRE like '%$criterio%' or SGD_CIU_APELL1 like '%$criterio%') and ";
				...
				$sqlWhere.="1=1 ";
				$query = "SELECT  '' COD_OEM, DPTO_CODI, MUNI_CODI, SGD_CIU_NOMBRE,SGD_CIU_APELL1 || ' ' ||  SGD_CIU_APELL2 APELLIDOS,
		...
		case '3':
		   {  if (strlen($criterio)) {
				...
				$sqlWhere.="1=1 ";
				$sqlWhere1.="1=1 ";
				$query = "SELECT IDENTIFICADOR_EMPRESA COD_OEM, NOMBRE_DE_LA_EMPRESA SGD_CIU_NOMBRE ,SIGLA_DE_LA_EMPRESA APELLIDOS, CODIGO_DEL_DEPARTAMENTO DPTO_CODI, 
				    CODIGO_DEL_MUNICIPIO MUNI_CODI, NIT_DE_LA_EMPRESA SGD_CIU_CEDULA, DIRECCION SGD_CIU_DIRECCION, TELEFONO_1 SGD_CIU_TELEFONO,  EMAIL SGD_CIU_EMAIL
					FROM BODEGA_EMPRESAS WHERE $sqlWhere
					UNION
				    SELECT SGD_OEM_CODIGO COD_OEM, SGD_OEM_OEMPRESA, SGD_OEM_REP_LEGAL, to_CHAR(DPTO_CODI), To_CHAR(MUNI_CODI),SGD_OEM_NIT,SGD_OEM_DIRECCION,SGD_OEM_TELEFONO,''
		...
		case '4':
			{   if (strlen($criterio)) $sqlWhere.=" USUA_NOMB like '%$criterio%' and ";
				...
				$sqlWhere.="1=1 ";
				$query ="SELECT '' COD_OEM, USUA_NOMB SGD_CIU_NOMBRE, '' APELLIDOS, DEPENDENCIA.DPTO_CODI DPTO_CODI, DEPENDENCIA.MUNI_CODI MUNI_CODI,
...
function tipodocumentos($username, $password, $codidepe, $codiserie, $codisubserie) {
	...
	$db = ADONewConnection('oci8'); # ej. 'mysql' o 'oci8' 
	//$db->debug = true;
	$db->Connect('172.20.2.14', 'fldoc', 'accion', 'acs_orcl');
...
function enviosplanilla($username, $password, $codplanilla, $fechaplanilla, $zona) {
	...
	if ($rs->RecordCount( ) > 0) {
		...
		$query = "SELECT SGD_RENV_CANTIDAD  Item, RADI_NUME_SAL Radicado, SGD_RENV_NOMBRE Destinatario, SGD_RENV_MPIO Municipio, SGD_RENV_DEPTO Depto,  
                    ...
                    Where $sqlWhere  SGD_FENV_CODIGO = $codplanilla and SGD_RENV_FECH >= TO_DATE('$fechaplanilla','dd/mm/YYYY:hh:mi:ssam') ";
...
function chkradicado($username, $password, $nroradicado) {
	...
	if ($rs->RecordCount( ) > 0) {
		...
		if ($nroradicado!='' ) {
		// 104 Codigo de la dependencia de Control Interno se debe filtrar por esa dependencia
		$query = "SELECT  a.RA_ASUN,  
              TO_CHAR(a.RADI_FECH_RADI,'DD/MM/YYYY') AS FECHALLEGADA, 
			  TO_CHAR(a.RADI_FECH_OFIC,'DD/MM/YYYY') AS FECHAOFICIO, 
              a.RADI_DEPE_RADI as DEPRAD, a.RADI_PATH as PATH,
              a.RADI_DEPE_ACTU as DEPACTU  
			FROM RADICADO a 
			WHERE a.RADI_NUME_RADI = $nroradicado ";
		
          
	 	$rsradicados = $db->Execute($query);
		$result= array();
		if (!$rsradicados->EOF) {
			$query  = "SELECT DEPE_NOMB FROM dependencia  WHERE depe_codi = ".$rsradicados->fields["DEPRAD"];
			$rsdepe = $db->Execute($query);

		   $query = "SELECT TO_CHAR(t.HIST_FECH,'DD/MM/YYYY') AS FECHAULT 
		            FROM HIST_EVENTOS t  
						WHERE radi_nume_radi = $nroradicado   and rownum = 1 order by hist_fech desc" ;
				$rshist = $db->Execute($query);

			    return  array('asunto'=> $rsradicados->fields["RA_ASUN"],
							  'fechallegada'=> $rsradicados->fields["FECHALLEGADA"],
							  'fechaoficio'=> $rsradicados->fields["FECHAOFICIO"],
							  'fechaultasig'=> $rshist->fields["FECHAULT"],
							  'path'=> $rsradicados->fields["PATH"],
							  'deprad'=> $rsdepe->fields["DEPE_NOMB"],
							  'estado'=> 2,
							  'descripcion'=> 'Existe y esta disponible para ser utilizada');

 		 }else {
      	return array('estado'=> 0,
             'descripcion'=> 'El numero de radicado no existe')	;
      }
	}
   	 return array('estado'=> 0,
              'descripcion'=> 'El numero de radicado es blanco')	;    	 
 }
  return array('estado'=> 0,
              'descripcion'=> 'El usuario no existe en  Orfeo ')	;
}
