    ...
    if 	($rsg->EOF)
	{
		//if($newId==true)
			...
			if ($nextval==-1)
			{
				...
				return " ERROR:  No se encontr� la secuencia sec_dir_direcciones ";
			}
...
/* Funcion que retorna los datos de un usuario registrado en Orfeo.
   Recibe como parametro de entrada el nombre de inicio de sesion(login) de orfeo
*/
...
function informacionRadicado($objDoc){
	...
	HIST.HIST_FECH as fechaRad
	FROM BODEGA_EMPRESAS BE,DEPARTAMENTO DPT,MUNICIPIO MUN, HIST_EVENTOS HIST
	...
	AND HIST.SGD_TTR_CODIGO='2'
	AND HIST.RADI_NUME_RADI='".$objDoc."'";
	...
	if($rs->EOF){
		...
		$sql = "SELECT HIST.HIST_FECH as fechaRad FROM HIST_EVENTOS HIST
		WHERE HIST.SGD_TTR_CODIGO='2' AND HIST.RADI_NUME_RADI='".$objDoc."'";
		$rs=$db->conn->Execute($sql);
		if($rs->EOF){
			return "ERROR: No se encuentra informacion de la empresa ni del radicado";	
		}
	}
