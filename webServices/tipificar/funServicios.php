...
function tipificarDocumento ($nurad, $usuario, $codiSRD, $codiSBRD, $codiTPR){
	...
	try{
		...
		/*Modificacion 16 Dic 08 Ing.Leonardo Chaves
	       Se incluye esta consulta para reflejar el tipo documental en el radicado(Ver consulta general y vista de expediente)
	       */
	       $sql='update radicado set tdoc_codi='.$codiTPR.' where (radi_nume_radi='.$nurad.')';
	       if(!$db->conn->Execute($sql)){
		   return "ERROR: Al actualizar el tipo documental en el radicado";
	       }
	       //Fin Modificacion 16 Dic 08 Ing.Leonardo Chaves
...
function isDocumentoTipificado ($radicado){
  global  $ruta_raiz;
  $db = new ConnectionHandler($ruta_raiz,'WS');     $sql = "select * from sgd_rdf_retdocf where radi_nume_radi = $radicado";
  $rs = $db->conn->Execute($sql);
    if (!$rs->EOF){
      return true;
  }     return false;
...
// Cambie TPR.SGD_TPR_ESTADO = 1 por (TPR.SGD_TPR_ESTADO = 1 OR TPR.SGD_TPR_ESTADO IS NULL)
	$sql =
