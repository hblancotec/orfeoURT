...
require 'libs/conexionServicioExterno.php';
                    ...
                    if ($cumplelogica) {
                        ...
                        if ($strFile) {
                            ...
                            try {
                                ...
                                $servicio = new servicioExterno($this->db);
                                ...
                                $zip = new ZipArchive();
                                $filename = BODEGAPATH."/tmp/Adjuntos_".$this->radicado.".zip";
                                
                                if ($zip->open($filename, ZipArchive::CREATE)!==TRUE) {
                                    $this->db->conn->FailTrans();
                                    $retorno = array('success' => FALSE, 'message' => "cannot open: $filename");
                                    return $retorno;
                                }
                                ...
                                $dataId = $this->codEnvio;
                                
                                //HACEMOS EL ENVIO DEL CORREO ELECTRONICO{}
							    //$objCorreo->agregarAdjunto($tmpRuta, $this->radicado . ".$ext");
								
								$asuntoc = iconv('iso_8859-1', 'utf-8', CORREOCERTIFICADO_ASUNTO . $this->radicado);
								//$asuntoc = CORREOCERTIFICADO_ASUNTO . $this->radicado;
								$cuerpoc = iconv('iso_8859-1', 'utf-8', CORREOCERTIFICADO_CUERPO);
								//$cuerpoc = CORREOCERTIFICADO_CUERPO;
								$cuerpo4 = iconv('iso_8859-1', 'utf-8', "Comunicación Oficial.\r\n La Unidad de Restitución de Tierras le envía este oficio mediante notificación certificada, De acuerdo a la Directiva Presidencial No. 04 de abril 3 del a�o 2012 y de conformidad con lo previsto en la Ley 1437 de 2011, por el cual se expide el C�digo de Procedimiento Administrativo y de lo Contencioso. \r\n ***Importante: Por favor no responda a este correo electr�nico. Esta cuenta no permite recibir correo.");
								//$cuerpo4 = "Comunicación Oficial.\r\n La Unidad de Restitución de Tierras le envía este oficio mediante notificación certificada, De acuerdo a la Directiva Presidencial No. 04 de abril 3 del año 2012 y de conformidad con lo previsto en la Ley 1437 de 2011, por el cual se expide el Código de Procedimiento Administrativo y de lo Contencioso. \r\n ***Importante: Por favor no responda a este correo electrónico. Esta cuenta no permite recibir correo.";
								//$objCorreo->enviarCorreo(array($this->correoe), null, null, $asuntoc, $cuerpoc);   //array("correo@certificado.4-72.com.co")
								
								$nombre = iconv('iso_8859-1', 'utf-8', $this->nombre);
								//$nombre = $this->nombre; 
								$response = $servicio->RegistrarMensaje($this->radicado, $asuntoc, $cuerpo4, $nombre, $this->correoe, $data, $this->radicado . ".zip", WSDL);
                                ...
                                if ($response['Resultado']) {
                                    ...
                                    $this->db->conn->FailTrans();
                                    $retorno = array('success' => FALSE, 'message' => "El correo del radicado " . $this->radicado . " no pudo ser enviado, intente nuevamente. Error: ". $response['Mensaje'] . " - " . $response['CodigoRespuesta']);
                                }
                            } catch (Exception $exc) {
                                $this->db->conn->FailTrans();
                                $retorno = array('success' => FALSE, 'message' => $exc->message);
                            }
                        } else
