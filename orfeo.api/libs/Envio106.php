...
require_once 'util/referenciaServicios/EmailServiceWCF/EmailServiceWCFClient.class.php';

class Envio106 extends Envios {
        ...
        if ($this->validarDatos()) {
            ...
            if ($datosRadicado && is_array($datosRadicado)) {
                ...
                if (isset($datosRadicado['RADI_PATH']) && strlen($datosRadicado['RADI_PATH']) > 0 && (strtolower($ext) == 'pdf' || strtolower(substr($ext, 0, 3)) == 'tif')) {
                    ...
                    if ($cumplelogica) {
                        $tmpRuta = RUTARAIZ . BODEGA . $datosRadicado['RADI_PATH'];
                        $strFile = file_get_contents($tmpRuta);
                        if ($strFile) {

                            $strFileEncoded64 = base64_encode($strFile);
                            $datosAttachment['Attachments'][] = Array("fileBase64Binary" => $strFileEncoded64, "nombreArchivo" => $this->radicado . ".$ext");

                            $ClienteEmailServiceWCF = new EmailServiceWCFClient();
                            $asuntoc = iconv('iso_8859-1', 'utf-8', "(R" . $this->radicado . ") (c) " . CORREOCERTIFICADO_ASUNTO . $this->radicado);
                            $cuerpoc = iconv('iso_8859-1', 'utf-8', CORREOCERTIFICADO_CUERPO);
                            $response = $ClienteEmailServiceWCF->enviarCorreo("notificaciones@dnp.gov.co", "Notificaciones", array($this->correoe), $asuntoc, $cuerpoc, NULL, NULL, json_encode($datosAttachment));
                            ...
                            if (!strpos(strtolower($response), "error")) {
                                ...
                                $retorno = array('success' => FALSE, 'message' => "El correo del radicado " . $this->radicado . " no pudo ser enviado, intente nuevamente.");
                        } else
