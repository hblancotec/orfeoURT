...
class usuarios{
  ...
  /**
	...
	function getUsuarios($idObjetoHtml,$depeCodi,$txAccion) {
		$response = new HTML_AJAX_Action();
			$iSql = "select (CASE u.SGD_ROL_CODIGO
                WHEN 0 THEN u.USUA_NOMB
                WHEN 1 THEN u.USUA_NOMB
                WHEN 2 THEN u.USUA_NOMB+'(Encargado)'
                END) as USUA_NOMB
                , u.USUA_CODI
								, u.SGD_ROL_CODIGO
			from usuario u
			where u.depe_codi=$depeCodi AND u.USUA_ESTA=1 ";
		if($txAccion){
			$iSql .= " AND (U.SGD_ROL_CODIGO IN (2,1)) ";
		}
		...
		while (!$rsUs->EOF)
		{
			$usuaCodi = $rsUs->fields["USUA_CODI"];
			...
			if($usuaRolCodi==1 || $usuaRolCodi==2) $indexSelected = $j;
			$j++;
			$rsUs->moveNext();
		}
