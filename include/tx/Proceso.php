...
//			$this->db->conn->debug = true;
 	...
 	/**
        ...
        **/
		...
		$rs = $this->db->conn->Execute($queryID);
...
//			$this->db->conn->debug = true;
	...
	function insertaEtapa(){
		...
		if($orden != null){	
			...
			$rsOrd = $this->db->conn->Execute( $queryOrdenET );
			$counter = 0;
			while( !$rsOrd->EOF )
			{
				$counter++;
		...
		$rs = $this->db->conn->Execute($queryYaExiste);
		...
		$rs = $this->db->conn->Execute($queryOrden);
	...
	function modificaEtapa( $etapaAModificar ){
		...
		if( $descripcion != null ){
			...
			if ( !$rsOrd = $this->db->conn->Execute( $queryOrdenET ) ){
...
//			$this->db->conn->debug = true;
	...
	function modificaArista( $aristaAModificar ){
        ...
        if ( !$rs = $this->db->conn->Execute( $queryAristMod ) ){
