 ...
 * @param  boolean Habilita/Deshabilita la caracteristica required.
 * @return string Cadena con el combo - False on Error.
 */
function Get_ComboOpc($dato1, $dato2, $dato3=true)
{	
	...
	else
	{
		...
		($dato2) ? $tmp2="Onchange='Actual(2)'" : $tmp2 = '';
		$this->flag = $rs->GetMenu('slc_cmb2',false,$tmp1,false,false,"id='slc_cmb2' class='select' $tmp2 $dato3");
...
{	
	$sql = "SELECT SGD_DEVE_DESC AS DESCRIP, SGD_DEVE_CODIGO AS ID, SGD_DEVE_ESTADO AS ESTADO FROM SGD_DEVE_DEV_ENVIO ORDER BY 1";
	$rs = $this->cnn->Execute($sql);
	if (!$rs)
		$this->vector = false;
	else
