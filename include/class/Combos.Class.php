...
/**
 * Esta clase se encrga de devolver los combos requeridos
 */
class Combos {
    ...
    public function __construct($ruta_raiz=".", $ruta_fuente=".") {
    ...
    function getUsuariosDep($dep,$usu=false){
        ($dep)?$slcDepe=$dep:$slcDepe=0;
        $sql = "SELECT ".$this->db->conn->concat("U.USUA_NOMB","'('","U.USUA_LOGIN","')'").", U.USUA_LOGIN FROM USUARIO U WHERE U.USUA_ESTA=1 AND U.DEPE_CODI=".$slcDepe;
        $rs_usu = $this->db->conn->Execute($sql);
        return $rs_usu->GetMenu2('slcUsua',$usu,"0:&lt;&lt;SELECCIONE",false,false,'id="slcUsua" class="select"');
    }
