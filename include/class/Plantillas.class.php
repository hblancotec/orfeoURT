...
class Plantilla
{
	var $error;
	var $db;
	var $arrayTpAyuda = array(1=>'Manuales',2=>'Plantillas Generales',3=>'Talento Humano',4=>'Servicio al Ciudadano');
	...
	function vistaDir($dir,$adm,$ruta_raiz="..")
	{
		...
		if($adm)
		{
			$tbl.="	<tr bordercolor='#FFFFFF' >
					...
					<tr bordercolor='#FFFFFF' >
						...
						<td align='center'  class='listado2' width='48%' >Tipo:&nbsp;&nbsp;".$this->getComboTipoAyuda('tipoAyuda',"0:Selecciones")."
					</tr>
					<tr bordercolor='#FFFFFF' >
		...
		if($dr)
		{
			...
			while (($carpetaTipoAyuda = readdir($dr)) !== false)
			{
				...
				if(filetype($dir . DIRECTORY_SEPARATOR . $carpetaTipoAyuda)=="dir" && ($carpetaTipoAyuda!="." && $carpetaTipoAyuda!=".." && $carpetaTipoAyuda!=".svn"))
				{
					$drDe=@opendir($dir . DIRECTORY_SEPARATOR . $carpetaTipoAyuda);
					$i=0;
					$band=true;
					while (($archivo = readdir($drDe)) !== false)
					{
						if($archivo!="." && $archivo!=".." && $archivo!=".svn")
						{
							...
							$tam  =round(filesize($dir. DIRECTORY_SEPARATOR .$carpetaTipoAyuda. DIRECTORY_SEPARATOR .$archivo)/1024,0);
							$tbl .="<tr bordercolor='#FFFFFF' >
									<td  class='listado2'>
									<a href='".BODEGAURL."/Ayuda/$carpetaTipoAyuda/$archivo' target='_blank'>$archivo</a>
									</td>";
							if($adm)
							{
								$tbl.="<td  class='listado2'>
										<a href=\"javascript:borrar('$carpetaTipoAyuda/$archivo')\"><center>Borrar</center></a>
										</td>";
							}
							$tbl.="</tr>";
						}
	...
	function borraArchivo($rut)
	{
		try {
			include("../../config.php");
			$cmd = unlink(BODEGAPATH . "Ayuda". DIRECTORY_SEPARATOR. $rut);
		}
		catch (Exception $e)
		{
	...
	function agregaArchivo($tipoAyuda)
	{
		try
		{
			...
			$sql="select ANEX_TIPO_EXT from anexos_tipo";
			...
			$uploadDir = BODEGAPATH . "Ayuda/".strtolower($this->arrayTpAyuda[$tipoAyuda])."/";
