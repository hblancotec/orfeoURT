...
class InformacionPoblacional
{
     ...
     *
     * @param array $datos
     *            Vector asociativo con todos los campos y sus valores.
     ...
     *
     * @param int $dato
     *            Id del Medio de Recepcion a eliminar.
     * @return boolean $flag False on Error /
     */
    function SetDelDatos($dato)
    {
        $this->cnn->BeginTrans();
        $ok = $this->cnn->Execute('DELETE FROM SGD_INF_INFPOB WHERE ID_INFPOB=' . $dato);
        if ($ok) {
     ...
     *
     * @param
     *            boolean Habilita/Deshabilita la 1a opcion SELECCIONE.
     * @param
     *            boolean Habilita/Deshabilita la validacion Onchange hacia una funcion llamada Actual().
     * @return string Cadena con el combo - False on Error.
     */
    function Get_ComboOpc($dato1, $dato2)
    {
        $sql = "SELECT SGD_INFPOB_DESC AS DESCRIP, ID_INFPOB AS ID FROM SGD_INF_INFPOB ORDER BY 1";
        $rs = $this->cnn->Execute($sql);
        if (! $rs)
            $this->flag = false;
        else {
            ($dato1) ? $tmp1 = ":&lt;&lt;SELECCIONE&gt;&gt;" : $tmp1 = false;
            ($dato2) ? $tmp2 = "Onchange='Actual(5)'" : $tmp2 = '';
    ...
    {
        $sql = "SELECT SGD_INFPOB_DESC AS DESCRIP, ID_INFPOB AS ID, SGD_INFPOB_ACTIVO AS ESTADO FROM MEDIO_RECEPCION ORDER BY 1";
        $rs = $this->cnn->Execute($sql);
        if (! $rs)
            $this->vector = false;
        else {
