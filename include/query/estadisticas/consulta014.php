...
switch($db->driver) {
	case 'mssqlnative':
        if ( $dependencia_busq != 99999) {
            $condicionE = "	AND USD.DEPE_CODI=$dependencia_busq AND
			...
			if($tipoDocumento=='9999') {
				...
				WHERE r.RADI_USUA_ACTU=b.USUA_CODI 
					AND r.RADI_DEPE_ACTU = USD.DEPE_CODI AND
                    b.USUA_LOGIN = USD.USUA_LOGIN AND
                    b.USUA_DOC = USD.USUA_DOC 
					$condicionE
					$whereTipoRadicado 
				...
				FROM RADICADO r 
                    ...
                    SGD_USD_USUADEPE USD
				WHERE r.RADI_DEPE_ACTU=USD.DEPE_CODI AND
                    b.USUA_LOGIN = USD.USUA_LOGIN AND
                    b.USUA_DOC = USD.USUA_DOC AND
                    1=1 $condicionE $whereTipoRadicado  
	...
	$queryEDetalle = "SELECT 
			...
			FROM RADICADO r
                ...
                SGD_USD_USUADEPE USD
		WHERE r.RADI_DEPE_ACTU=$dependencia_busq AND
            USD.DEPE_CODI=$dependencia_busq AND
