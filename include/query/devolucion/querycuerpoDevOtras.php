<?php
	// Consultas de verficacion previa a la radicacion
	switch($db->driver) {
	 case 'mssqlnative':
		$sqlConcat = $db->conn->Concat("convert(char(15),a.radi_nume_sal,0)","'-'","cast(a.sgd_renv_codigo as varchar)");
		$isql = 'select \'4\' AS CHU_ESTADO,' . 0 . ' AS HID_DEVE_CODIGO,
                ...
                where sgd_deve_codigo is null and' .
                    ...
                    $sql_masiva . ' and
                    c.radi_nume_radi= a.radi_nume_sal and
                    c.radi_depe_actu = USD.depe_codi AND
                    d.USUA_DOC = USD.USUA_DOC AND
                    d.USUA_LOGIN = USD.USUA_LOGIN AND
                    c.radi_usua_actu = d.usua_codi
