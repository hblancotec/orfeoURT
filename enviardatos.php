<?php
session_start();
if (!$_SESSION['dependencia'] or !$_SESSION['nivelus'])  include "./rec_session.php";
?>
<html>
<head>
 					 ...
 					 function devolver_rad($chkd,$krd,$cnivel)
							 {
								 ...
								 $isql_hl= "select a.RADI_USU_ANTE,
                                                    b.USUA_CODI,
                                                    b.USUA_NOMB,
                                                    b.DEPE_CODI,
                                                    a.RADI_DEPE_ACTU,
								 				 ...
								 				 from radicado a,
                                                        usuario b
											   where b.USUA_LOGIN (+) =a.RADI_USU_ANTE AND
                                                    a.RADI_NUME_RADI=$chkd";
   ...
   $enviara="9";
		...
		<?php
			  if (session_id()) {
				if($enviardoc=="REALIZAR"){
				  ...
				  if ($enviara==20){
				    $iisql = "delete from informados
                                where radi_nume_radi in $radicados and
				  ...
				  if($enviara!=21) {
				    if ($enviara==9) {
						...
						$carp_per = 0;
						if($EnviaraV=="VoBo") {
                            $carp_codi=11; $observa = "VoBo. $observa";
                        }
						$isql = "update radicado set RADI_USU_ANTE='$krd',
                                            RADI_DEPE_ACTU = $depsel,
                                            RADI_USUA_ACTU = $codus,
                                            CARP_CODI = $carp_codi $cadenanivel,
					...
					if($reasignarnivel) $cnivel = 1; else $cnivel = 0;
  					...
  					if($chk8) {
						if($enviara==21){
                            $us_ant_nombre .= "<br>($chk8)".devolver_rad($chk8,$krd,$cnivel);
                        }
				  ...
				  ?><br>
        <span class="tituloListado">ACCION REQUERIDA COMPLETADA</span> <?php
				  $nomostrar = "jhlc";
				}
	//echo "Accion a realizar $enviara " ;
     ...
     $isql = "select a.*,
                ...
                from radicado a,
                    tipo_remitente b
                where a.trte_codi = b.trte_codi (+) and
                        a.RADI_NUME_RADI in$radicados";
	...
	if($nomostrar!="jhlc")
	 {
			...
			<select name=codus class=e_buttons> <?php
	DO
		{
  ...
  <tr class='etexto2'>
	...
	<?php
	 ...
	 {   $procard = str_replace(",","<br>",$procradi);
        ...
        <span class="etextomenu">ORIGEN :</span> <?php
	 }
	 ?>
