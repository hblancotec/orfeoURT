...
class OneToOneProxy(object):
    ...
    def __init__(self, key, remote_class, remote_property_name):
        ...
        # jcr compatible con attribtes sql orm.attributes.py
        self.parententity = remote_class
        self.property     = getattr(remote_class, remote_property_name, None)
        self.property.key = self.key
...
class OneToOneMapperProperty(MapperProperty):
    ...
    def do_init(self):
        ...
        # jcr
        prop = self.relation.mapper.get_property(self.remote_property_name)
        if (getattr(prop, 'columns', None) == None):
           # es tambien OneToOneMapperProperty 
           self.column = prop.column
        else:    
           self.column = prop.columns[0]
    ...
    def do_init(self):
        ...
        for prop_key, prop in self.mapper._Mapper__props.iteritems():
            ...
            if prop_key in self.fields:  
               ...
               prop_attr = self.prefix + "_" + prop_key
             
               #setattr(self.parent.class_, prop_key, proxy_property)
               setattr(self.parent.class_, prop_attr, proxy_property)
               ...
               self.parent._init_properties[prop_attr] = mapper_property
               self.parent._compile_property(prop_attr, mapper_property, True)
