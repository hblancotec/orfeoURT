...
include_once 'dataCommon.php';
	...
	switch ($evento) {		
	    ...
	    case 1:             	    
	    	...
	    	//si alguno de los siguientes parametros no esta, salga.
                        $depenUsua = !empty($selectDepe)?$selectDepe:$depenUsua;
			...
			$sql1 = "select
						DISTINCT (RIGHT('000' + convert(varchar,su.sgd_sbrd_codigo),3)+' - '+su.sgd_sbrd_descrip)AS DETALLE
						,RIGHT('000' + convert(varchar,su.sgd_sbrd_codigo),3) AS CODIGO_SUBSERIE					
					from
						sgd_mrd_matrird m
		...
		case 2:
			...
			$sql2 = "	SELECT	DISTINCT (RIGHT('0000' + CONVERT(VARCHAR,T.SGD_TPR_CODIGO),4)+' - '+T.SGD_TPR_DESCRIP) AS DETALLE,
								RIGHT('0000' + CONVERT(VARCHAR,T.SGD_TPR_CODIGO),4) AS CODIGO_SUBSERIE          		
						FROM	SGD_MRD_MATRIRD M
								JOIN SGD_TPR_TPDCUMENTO T ON
									T.SGD_TPR_CODIGO 	= M.SGD_TPR_CODIGO
						WHERE	M.DEPE_CODI = $depenUsua AND
								M.SGD_MRD_ESTA = 1 AND 
								M.SGD_SRD_CODIGO = $selectSerie AND 
								M.SGD_SBRD_CODIGO = $selectSubSerie
						ORDER BY DETALLE";
		...
		case 3:
			...
			$sql1 = "select
						DISTINCT (RIGHT('000' + convert(varchar,su.SGD_SRD_CODIGO),3)+' - '+su.SGD_SRD_DESCRIP)AS DETALLE
						,RIGHT('000' + convert(varchar,su.SGD_SRD_CODIGO),3) AS CODIGO_SERIE	
					from
						sgd_mrd_matrird m
						, SGD_SRD_SERIESRD su
					where
		...
		case 6:
			...
			$sql	= "	SELECT
 							(RIGHT('000' + convert(varchar,CU.SGD_DCAU_CODIGO),3)+' - '+CU.SGD_DCAU_DESCRIP)AS DETALLE
    ...
    case 7: //Buscar expedientes	
		...
		while(!$salida->EOF) {
			...
			$result[]= array("nombre" => $salida->fields["DETALLE"]
