...
include_once	("confSmarty.php");
	...
	$sql_depe = "SELECT
				      ...
				      ,RIGHT('000' + convert(varchar,DE.DEPE_CODI),3)+ 
					  			' - ' + DE.DEPE_NOMB AS NOMBRE
				FROM
				    DEPENDENCIA DE";	
	...
	$sql1			=	"SELECT
							DISTINCT (RIGHT('000' + convert(varchar,S.SGD_SRD_CODIGO),3)+' - '+S.SGD_SRD_DESCRIP)AS DETALLE
						    ,RIGHT('000' + convert(varchar,S.SGD_SRD_CODIGO),3) AS CODIGO_SERIE
						FROM
							SGD_MRD_MATRIRD M
							, SGD_SRD_SERIESRD S
						WHERE
			                M.DEPE_CODI 			= ".$_SESSION['dependencia'].
	...
	$smarty->assign("depecodi"		, $_SESSION['dependencia']	); //dependencia del usuario actual	
