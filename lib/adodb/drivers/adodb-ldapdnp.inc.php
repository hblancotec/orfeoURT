...
class ADODB_ldapdnp extends ADOConnection
{

    var $databaseType = 'ldapdnp';

    var $dataProvider = 'ldapdnp';
    ...
    function _connect($host, $username, $password, $ldapbase)
    {
        ...
        if ($this->_connectionID) {
            ...
            if (! $bind) {
                $er = ldap_error($this->_connectionID);
                switch (ldap_error($this->_connectionID)) {
                ...
                $this->_errorMsg = $msgError;
                @ldap_close($this->_connectionID);
            }
            return $bind;
        } else {
            throw new Exception("NO HAY CONEXI&OACUTE;N A SERVIDORES LDAP.");
        }
    }
 ...
 * --------------------------------------------------------------------------------------
 */
class ADORecordSet_ldapdnp extends ADORecordSet
{
