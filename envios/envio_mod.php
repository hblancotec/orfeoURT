...
function solonumeros()
{
 ...
 if(jh)
 {
    var1 =  parseInt(jh);
		if(var1 != jh)
		{
			alert('Sï¿½o introduzca numeros en el campo de planilla.' );
			//document.forma.submit();
			return false;
		  }else{
		    document.forma.submit();
...
if(!$reg_envio)
{
    ...
    //  Buscamos en SGD_DIR_DRECCIONES el radicado seleccionado y lo comparamos con el pais de la variable de sesión codlocal
	...
	else	// Si el país pertenece a América, osea grupo 1 o al grupo 2
	{ 	$var_grupo = 2;
...
//	echo "<hr>Radicados";
   ...
   if (($reg_envio) AND (!$envio_peso or strlen($valor_unit) == 0) ) die ("<p><hr><b><center><span class=etexto>Debe Colocar el Peso para poder Enviar el Radicado  Archivo anexado correctamente<br></br></span><hr>");
   if(!$reg_envio)
   {
    $i = $iii;
   ?>
