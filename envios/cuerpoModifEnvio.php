...
$dia_ini = date("d");
$ano_ini = date("Y");
if(!$_POST['fecha_ini']) $fecha_ini = "$ano_ini/$mes_ini/$dia_ini";
...
<?php
 ...
 if ($estado_sal==4) {
	...
	if (!$_POST['dep_sel'] && !$_GET['dep_sel']) {
	    $dep_sel = $_SESSION['dependencia'];
	}
	elseif ($_POST['dep_sel'] && !$_GET['dep_sel']) {
 ...
 if($busq_radicados_tmp)  {
   $where_fecha=" ";
    ...
    $where_fecha = " (a.SGD_RENV_FECH >= '". $fecha_ini ." 00:00:00' and a.SGD_RENV_FECH <= '". $fecha_fin." 23:59:59') " ;
