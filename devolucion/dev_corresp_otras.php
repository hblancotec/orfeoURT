...
$encabezado_i  = "estado_sal=$estado_sal";
$encabezado_i .= "&motivo_devol=$motivo_devol";
$encabezado_i .= "&estado_sal_max=$estado_sal_max";
$encabezado_i .= "&pagina_sig=$pagina_sig&dep_sel=$dep_sel&krd=$krd";
$realizarDev = (!empty($_POST['realizarDevolucion'])) ? $_POST['realizarDevolucion'] : null;
$devolver_rad = (!empty($POST['realizarDev'])) ? "CONFIRMAR DEVOLUCION": null;
?>
<html>
<head>
<link rel="stylesheet" href="../estilos/orfeo.css">
<script>
    function verificacionMotivo() {
        escogioMotivo = document.new_product.motivo_devol.value;
        if (escogioMotivo == 0) {
            alert('Elija un Motivo de devolucion.');
            return false;
        } 
        document.new_product.realizarDevolucion.value = true;
        seleccionado = verificacion();
        if (seleccionado) {
    ...
    function verificacion () {
        numeroEscogido  = 0;
        numeroTotal     = document.new_product.elements.length;
        ...
        if (numeroEscogido == 0) {
            alert ("debe seleccionar por lo menos un Item para realizar la operacion");
            return false;
        }
        return true;
    }
...
<table width="100%" class='borde_tab' cellspacing="5">
    <tr>
        <td height="30" valign="middle"   class='titulos5' align="center">DEVOLUCI&Oacute;N DE DOCUMENTOS</td>
    </tr>
</table>
...
<center>
<?php
    if( (!isset($_POST['devolver_rad'])) or $_POST['motivo_devol']== 0) {
?>
...
<?php
		...
		$sql = "select $sqlConcat,
			...
			print $rsDep->GetMenu2("motivo_devol",
                                    "$motivo_devol",
  ...
  <tr>
    ...
    <tr>
        <td height="26" colspan="2" valign="top" class='titulos2'>
            <center>
                <input type="submit" name="devolver_rad"  value="CONFIRMAR DEVOLUCION" class="botones_largo" onClick="verificacionMotivo();">
            </center>
        </td>
        <input type="hidden" id="realizarDevolucion" name="realizarDevolucion"  value="">
    </tr>
</table>
</center>
<table><tr><td></td></tr></table>
<?php
    ...
    if(!$_POST['radicados_dev']  or $_POST['motivo_devol']==0) {
    ...
    if(isset($_POST['devolver_rad'])  and isset($_POST['motivo_devol']) ) {
		...
		while ($i < $num) {
            ...
            $isql_hl= "insert into hist_eventos(DEPE_CODI,
                                                ...
                                                USUA_DOC,
                                                SGD_TTR_CODIGO)
                                        values (".$_SESSION['dependencia'].",
                                                $systemDate,".
                                                $_SESSION['codusuario'].",
                                                $chkt,
                                                'Devolucion ($motivo). $comentarios_dev',
                                                NULL,
                                                '".$_SESSION['usua_doc']."',
                                                28)";
            ...
            if($radi_nume_padre != $radi_nume_deri and  $radi_nume_padre) {
                $isql_hl= "insert into hist_eventos(DEPE_CODI,
                                                ...
                                                values (".$_SESSION['dependencia'].",
                                                    ...
                                                    $_SESSION['codusuario'].",
                                                    $radi_nume_padre,
                                                    'Devolucion($chkt, $motivo). $comentarios_dev',
                                                    '',
                                                    '".$_SESSION['usua_doc']."',
                                                    28)";
                $rs = $db->conn->Execute($isql_hl);
...
<?php
    ...
    $isql = 'select ' . $nombre . ' as "DAT_Numero Radicado",
                ...
                where a.radi_nume_sal=b.radi_nume_radi
			        '.$condicion.'';
    $rs = $db->conn->Execute($isql);
	...
	//echo $radicados_dev;
        ...
        echo '<span class="etexto"><b>No se actualizaron los registros <br>Debe seleccionar un tipo de devoluci&oacute;n<br>';
        echo '<input type="hidden" name="devolucion_rad" value="si">';
        }
    }
    if(!isset($_POST['devolver_rad'])  or !isset($_POST['motivo_devol'])) {
        $sqlFecha = $db->conn->SQLDate("d-m-Y H:i A","a.SGD_RENV_FECH");
        include "$ruta_raiz/include/query/devolucion/querydev_corresp_otras.php";
        ...
        $condicion = $radicados_devOrginal;
        $isql = 'select ' . $nombre . '    as  "DAT_Numero Radicado", 
                        a.radi_nume_sal    as "HID_Radicado", 
                        ...
                        sgd_renv_valor     as  "Valor Unitario",
                        a.sgd_renv_codigo  as "HID_sgd_renv_codigo",
                        '.$nombre.' "CHK_CHKANULAR"
                    from SGD_RENV_REGENVIO a,
                            radicado b
                    where a.radi_nume_sal = b.radi_nume_radi
                            '.$condicion.'';
        $rs = $db->conn->Execute($isql);
        $pager = new ADODB_Paginacion($db, $isql, 'adodb', false, $orderNo, $orderTipo);
        $pager->toRefLinks = $linkPagina;
        $pager->toRefVars = $encabezado;
        $pager->checkAll = true;
        $pager->checkTitulo = false;
