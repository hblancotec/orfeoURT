...
<?php
    $ruta_raiz = ".";
	if(!$_SESSION['dependencia'] or !$_SESSION['krd'] or strlen(trim($_SESSION['usua_nomb'])) < 1 ) include "./rec_session.php";
  ...
  ?> 
		...
		<?php if ($codusuario!=1){?>
		   ...
		   <?php } else{?>
			comboUsuarioDependencia(document.formulario,<?php echo ($dependencia_busq); ?>,'codus');
		<?}?>
		</script>
		</TR>
			<Tr> 
			...
			<?php  if($tarea==0)$datoss= " selected "; else $datoss= " ";?>
			<option value=0 '<?=$datoss?>'>Mostrar Registros</option>
			<?php  if($tarea==1)$datoss= " selected "; else $datoss= " ";?>			
    ...
    <TD>
			<?php if ($agrupar_tipo==1){ $datoss = " checked "; }else{ $datoss = " "; }?>
