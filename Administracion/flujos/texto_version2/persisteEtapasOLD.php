...
if ( $usuarioGrafico == $krd ) {
	...
	$rsPasswd = $db->conn->Execute( $selectPasswd );
		...
		if ( $_POST['infoNodos'] ){
			...
			//Procesamos Nodos
				...
				//Se lee el proceso al que se le está creando flujo
					...
					$rsProc = $db->conn->Execute( $maxProc );
				...
				//Aristas
//		var_dump($documento->flujo->arista);
	...
	//
				 ...
				 $arista->initArista( $nodos [ $documento->flujo->arista->origen ], $nodos [ $documento->flujo->arista->destino ], 'Arista ' . $value-> arista->atts->id , 0, 0, 0,0,0, 0, $procesoSelected, 1, 0 );	
		 		 ...
		 		 if ($documento->flujo->arista != NULL ){
	                if( is_array($documento->flujo->arista)){
	                        foreach ($documento->flujo->arista as $clave=> $value ){
	                                $arista->initArista( $nodos [ $value->origen ], 
	                                                $nodos [ $value->destino ], 
	                                                'Arista ' . $value-> arista->atts->id,0,0,0,0,0,0, 
	                                                $procesoSelected, 1, 0 );    
	                                $resultadoInsercionAr = $arista-> insertaArista(  );
                    				echo $resultadoInsercionAr;
	                        }   
	                }else{
	                        echo "1 sola Arista luego no array";
	                         $arista->initArista( $nodos [ $documento->flujo->arista->origen ],
	                                                $nodos [ $documento->flujo->arista->destino ],
	                                                'Arista ' . $value-> arista->atts->id,0,0,0,0,0,0,
	                                                $procesoSelected, 1, 0 );
                                    $resultadoInsercionAr = $arista-> insertaArista(  );
                    				echo $resultadoInsercionAr;
	...
	echo "Error:Usuario No Concuerda. Datos Que llegan desde applet: Usuario:$usuarioGrafico Password: $passwordUsuarioGrafico - Usuario que está loggeado: $krd";
}
?>