				...
				{	$ADODB_COUNTRECS = true;
					...
					{	$conn->BeginTrans();
						...
						{	$ok = $conn->Execute('DELETE FROM DEPARTAMENTO WHERE ID_PAIS=?',$record['ID_PAIS']);
							...
							{	$arr[] = $record['ID_PAIS'];
                                $arr[] = $record['ID_CONT'];
								$ok = $conn->Execute('DELETE FROM SGD_DEF_PAISES WHERE ID_PAIS=? AND ID_CONT=?',$arr);
							}
						}
...
<tr bordercolor="#FFFFFF"> 
	<td width="3%" align="justify" class="info" colspan="3" bgcolor="#FFFFFF"><b>NOTA: </b> Para una estandarización en los c&oacute;digos de pa&iacute;ses utilicemos los sugeridos por la ISO.  <a href="http://es.wikipedia.org/wiki/ISO_3166-1" target="_blank" class="vinculos">enlace</a></td>
</tr>
<?php
if ($error)
{	echo '<tr bordercolor="#FFFFFF"> 
	...
	{	case 1:	//NO CONECCION A BD
		...
		case 4:	//INSERCION REALIZADA
				echo "País creado satisfactoriamente!!";break;
...
{	var strMensaje = "Por favor ingrese las datos.";
	...
	{	if(confirm("Esta seguro de borrar el registro ?\n La eliminación de este pais incluye sus Dptos y municipios."))
		{	document.form1.submit();	}
		else
		{	return false;	}
	}
