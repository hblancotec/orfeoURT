			...
			Case 'Eliminar':
				{	$ADODB_COUNTRECS = true;
					$sql = "SELECT	RADI_NUME_RADI
							...
							WHERE	DPTO_CODI = ".$record['DPTO_CODI']." AND 
									ID_PAIS = ".$_POST['idpais'] ." AND
									ID_CONT = ".$_POST['idcont'];
					$rs = $conn->Execute($sql);
					
					$sql2 ="SELECT	MUNI_CODI 
							FROM	MUNICIPIO 
							WHERE	DPTO_CODI = ".$record['DPTO_CODI']." AND 
									ID_PAIS = ".$_POST['idpais'] ." AND
									ID_CONT = ".$_POST['idcont'];
...
{	echo '<tr bordercolor="#FFFFFF"> 
	...
	{	case 1:	//NO CONECCION A BD
		...
		case 4:	//INSERCION REALIZADA
			...
			break;
		case 5:	//IMPOSIBILIDAD DE ELIMINAR DEPARTAMENTO, ESTÁ LIGADO CON RADICADOS
			echo "No se puede eliminar departamento, se encuentra ligado a direcciones.";
			break;
		case 5:	//IMPOSIBILIDAD DE ELIMINAR DEPARTAMENTO, TIENE MUNICIPIOS ASOCIADOS
			echo "No se puede eliminar departamento, se encuentra ligado a direcciones.";
			break;	
	}
	echo '</td></tr>';
...
{	var strMensaje = "Por favor ingrese las datos.";
	...
	{	if(confirm("¿Esta seguro(a) de borrar el Departamento?"))
		{	document.form1.submit();	}
		else
		{	return false;	}
	}
