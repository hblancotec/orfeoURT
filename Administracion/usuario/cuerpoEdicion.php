...
if (count($_SESSION) == 0) {
    ...
    if (!$dep_sel) $dep_sel = $_SESSION['depecodi'];
    ...
    $orden_cambio = $_GET['orden_cambio'];
    ...
    $encabezado .= "&pagina_sig=$pagina_sig&accion_sal=$accion_sal";
    $encabezado .= "&radSelec=$radSelec&dependencia=$dependencia";
    $encabezado .= "&dep_sel=$dep_sel&selecdoc=$selecdoc";
    $encabezado .= "&nomcarpeta=$nomcarpeta&orderTipo=$orderTipo&orderNo=";
    $linkPagina  = "$PHP_SELF?$encabezado&radSelec=$radSelec";
    $linkPagina .= "&accion_sal=$accion_sal&nomcarpeta=$nomcarpeta";
    $linkPagina .= "&orderTipo=$orderTipo&orderNo=$orderNo";
...
<link rel="stylesheet" type="text/css" href="js/spiffyCal/spiffyCal_v2_1.css">
  <form name="formEnviar" action='./adm_usuarios.php?<?=$encabezado?>&usModo=2' method="post">
<?php
	...
	$sqlConcat = $db->conn->Concat("a.radi_nume_sal",
                                    "'-'",
                                    "a.sgd_renv_codigo",
                                    "'-'",
                                    "a.sgd_fenv_codigo",
