...
<?php
			if (($depe_codi_padre and $codusuario==1) or $codusuario!=1) {
                echo '<input type="submit" value="VoBo" name="EnviaraV" id="Enviar" class="ebuttons2">';
			}
?>
...
<?php
		    /* si esta en la Carpeta de Visto Bueno no muesta las opciones de reenviar
			   */
			if($mostrar_opc_envio==0) {
                echo '<td width="94%"  align="right" class="grisCCCCCC">';
                //ora_commiton($handle);
		...
		$subDependencia = $db->conn->substr ."(depe_nomb,0,50)";
                ...
                $sql = "select carp_desc as nomb_carp,
                                carp_codi as codi_carp,
                                0 as orden 
                        from carpeta
                        union
                        select $nombreCarpeta,
                        codi_carp,1 as orden			 
                         from carpeta_per 
