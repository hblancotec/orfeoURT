...
require "crea_combos_universalesNEW.php";
$tip3img = $_SESSION['tip3img']; $tip3desc = $_SESSION['tip3desc']; $tip3Nombre = $_SESSION['tip3Nombre'];
$no_tipo = "true";
...
function grabarDirecciones(radiNumeRadi){
  ...
  */
  ...
  remoteRad.insertDireccionAjax(radiNumeRadi,2,0,datosRad['grbNombresUs'],datosRad['ccDocumento'],
                      datosRad['muniCodi'],datosRad['dptoCodi'],datosRad['idPais'],datosRad['idCont'],
                      funCodigo, oemCodigo, ciuCodigo, espCodigo,
                      datosRad['direccion'],datosRad['codpostal'],datosRad['dirTelefono'],datosRad['dirMail'],datosRad['dirNombre']
                      );
  }
...
function modificar_doc() {
    if (    document.formulario.documento_us1.value && 
    	    (document.formulario.codpostal_us1.value.length > 0) && 
    	    (!(isNaN(parseInt(document.formulario.codpostal_us1.value))))
    )
    {
	    ...
	    alert("Remitente/Destinatario (incluyendo c\xf3digo postal) son obligatorios ");
    }
}
...
<?php
  ...
  if($Buscar) {
		...
		$fRad = $db->conn->SQLDate('d-m-Y','a.RADI_FECH_RADI');
...
<tr class="e_tablas">
	...
	<input type="text" name='prim_apel_us<?=$i ?>' id='prim_apel_us<?=$i ?>' value='<?=$papel ?>' class="tex_area"  readonly  size="40">
<?php
	$cp="codpostal_us";
?>
	</td>
...
<tr class=e_tablas>
	...
	<td width="10%" class="titulos5"  align="right">
	 ...
	 <td bgcolor="#FFFFFF"  class="listado5">
	  <INPUT type=text name='codpostal_us<?=$i ?>'  id='codpostal_us<?=$i ?>' value='<?=${$cp.$i} ?>'  <?=$bloqEdicion?> class="tex_area" size="7" maxlength="7">&nbsp;&nbsp;
	  <a href="http://www.codigopostal4-72.com.co/CodigosPostales/" style="border: 0" target="_blank" >
			<img title="C&oacute;d. Postales Colombianos" alt="C&oacute;d. Postales Colombianos" src="../imagenes/logo472.png">
		</a>
	 </td>
</tr>
 <tr class=e_tablas>
	...
	<td bgcolor="#FFFFFF"  class="listado5">
		<INPUT type=text name='mail_us<?=$i ?>'  id='mail_us<?=$i ?>' value='<?=$mail ?>' readonly class="tex_area" size=35>
	</td>
...
<?php
	...
	*/
	...
	(${$var_cnt}) ? $contcodi = ${$var_cnt} : $contcodi = 0 ;
	(${$var_pai}) ? $paiscodi = ${$var_pai} : $paiscodi = 0 ;
	(${$var_dpt}) ? $deptocodi = ${$var_dpt} : $deptocodi = 0 ;
	(${$var_mcp}) ? $municodi = ${$var_mcp} : $municodi = 0 ;

	//	Visualizamos el combo de continentes.
	echo "<SELECT NAME=\"idcont$i\" ID=\"idcont$i\" CLASS=\"select\" >";
	if ($contcodi) {
...
<?php
	...
	echo "<SELECT NAME=\"idpais$i\" ID=\"idpais$i\" CLASS=\"select\" >";
	if ($paiscodi) {
		foreach ($vpaisesv as $key => $value) {
		    echo "<option value='" . $value['ID1'] . "' selected >" . $value['NOMBRE'] . "</option>";
	}	}
	echo "</SELECT>";
?>	</td>
<tr>
	...
	<td width="20%" bgcolor="#FFFFFF"   class="listado5">
<?php
	echo "<SELECT NAME=\"codep_us$i\" ID=\"codep_us$i\" CLASS=\"select\" >";
	if ($deptocodi) {
		foreach ($vdptosv as $key => $value) {
		    echo "<option value='" . $value['ID1'] . "' selected >" . $value['NOMBRE'] . "</option>";
	}	}
	echo "</SELECT>";
?>
...
<?php
	...
	if ($municodi) {
		foreach ($vmcposv as $key => $value) {
		    echo "<option value='" . $value['ID1'] . "' selected >" . $value['NOMBRE'] . "</option>";
	}	}
	echo "</SELECT>";
	$municodi = 0;$contcodi=0;
	$muninomb = "";$paiscodi=0;
	$deptocodi = 0;
?>
...
if ($parametroTip == True)
{
  ?>
    ...
    <td width="25%" valign="top" class="listado5"> <font color="">
    <input name="hoj" id="hoj" type=hidden value="<?php echo $hoj; ?>">
<?php
...
<table width=100% border="0" cellspacing="1" cellpadding="1" class="borde_tab" align="center">
	<tr>
		<td class="titulos5" width="25%" align="right">
			<font color="" face="Arial, Helvetica, sans-serif" class="etextomenu"> Observaci&oacute;n </font>
		</td>
		<td width="100%" class="listado5" >
			<textarea name="obs" id="obs" cols="70" class="tex_area" rows="1" ><?php echo htmlspecialchars(stripcslashes($obs)); ?></textarea>
		</td>
	</tr>
</table>
<table width=100% border="0" cellspacing="1" cellpadding="1" class="borde_tab" align="center">
...
$tip_doc += 0;
$numdoc .= '';
$numdoc = (strlen($numdoc)>0) ? trim(substr($numdoc,0,13)) : '' ;
$codieesp = ($codieesp>0) ? $codieesp : 0;
$radicadopadre += 0;
$long = strlen($cod);
$codep += 0;
$tel = (int) $tel;
...
$med +=0;
	...
	$fecha_gen_doc_YMD = substr($fecha_gen_doc,0 ,2)."-".substr($fecha_gen_doc,3 ,2)."-".substr($fecha_gen_doc,6 ,4);
	$rad->radiFechOfic =  $fecha_gen_doc_YMD;
	if(!$radicadopadre)  $radicadopadre = null;
	$rad->radiNumeDeri = $radicadopadre;
	$rad->radiPais =  "'$pais'";
	$rad->descAnex = $ane;
	$rad->raAsun   = iconv('ISO-8859-1', 'UTF-8', $asu);
	$rad->radiDepeActu = $coddepe ;
	$rad->radiUsuaActu = $radi_usua_actu ;
	$rad->trteCodi =  $tip_rem;
	$rad->tdocCodi = $tdoc;
	...
	if($resultado)
	{
		...
		$radicadosSel[] = $nurad;
		$codTx = 11;
		$hist->insertarHistorico($radicadosSel,  $dependencia , $codusuario, $coddepe, $radi_usua_actu, $obs, $codTx);
	}
	else {
	    echo "<center><font color=red>Radicado No $nurad NO fue Modificado, $resultado </font></center>";
	}
}
...
<?php
	...
	<table class='borde_tab' ><tr><td class=listado2>
