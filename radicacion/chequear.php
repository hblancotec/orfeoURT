 ...
 * @version     3.0
 * var $faxPath; Variable que trae la ruta actual de un fax entrante.
 */
...
if(trim($faxPath)) {
	$iSql= "SELECT	SGD_RFAX_FAX, 
			...
			WHERE	SGD_RFAX_FAX='$faxPath' AND 
					USUA_LOGIN='$krd'";
	//$db->conn->debug = true;
	...
	if($krdTmp!=$krd) {
		...
		?>
				<hr>
					<font color=red>
						POR FAVOR VERIFIQUE CON <?=$krdTmp?> QUE EL FAX QUE TOMO NO SEA RADICADO POR SEGUNDA VEZ, YA OTRA PERSONA (<?=$krdTmp?>) lo habia radicado
					</font>
				<hr>
		...
		?>
				<hr>
					<font color=red>
						CUIDADO !!!!! ESTE FAX YA APARECE CON FECHA DE RADICACION !!!!!!! <?=$krdTmp?> CONSULTE CON (<?=$krdTmp?>) FECHA DE RADICACION <?=$fechRadicacion?>
					</font>
				<hr>
		<?php
			}
		...
		$iSql= "INSERT INTO	SGD_RFAX_RESERVAFAX	(	sgd_rfax_codigo, 
													sgd_rfax_fax, 
													usua_login, 
													sgd_rfax_fech) 
						VALUES	(	$codigoFax
...
if(trim($nombres)) {
	$where_ciu .= " and ".$db->conn->Concat("UPPER(d.SGD_DIR_NOMREMDES)","UPPER(d.SGD_DIR_NOMBRE)")." LIKE '%$nombres%' ";
}/* 
if(trim($nombres)) {
...
$dato=2;
echo "</table>";
if($primera!=1 and $Submit=="BUSCAR" and ($buscar_por_cuentai or
