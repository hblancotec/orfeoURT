 ...
 * @date       31. March 2008
 * @version    $Id: csql.php,v 1.3 2009/05/21 14:15:25 cgonzalez Exp $
 */
...
class csql {
	...
	/**
	  ...
	  * @param     string $file
	  */
	protected function getOdb($engine, $file, $servidor, $usuario, $contrasena, $db) {
		switch($engine) {
			case "sqlite":
				if("/" !== $file[0]) {
				   $file = realpath(".") . "/$file";
			...
			case "mssql":
                  ...
                  $odb = new PDO("mssql:dbname=$db;host=$servidor",$usuario ,$contrasena);
                  //$odb = new PDO("mssql:dbname=$dbname;host=$dbhost",$dbuser,$dbpass);
                  $odb ->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		...
		$sql .= implode(",", $fields);
		   ...
		   $sql .= isset($groupBy) && $groupBy ? " group by $groupBy" : "";
			...
			$sql .= is_null($dir) ? "" : " $dir";
                   ...
                   $orderby = "";
                   if(!is_null($sort)) {
                      $orderby .= " over (order by $sort";
                      $orderby .= is_null($dir) ? "" : " $dir";
                      $orderby .= ")";
                   }
                   $sql .= " from (SELECT ROW_NUMBER()" . $orderby ." AS Row, ";
                   $sql .= implode(",", $fields) . " from $table) as log ";
                   $sql .= "WHERE Row >= $start AND Row <= $limit";
                }
