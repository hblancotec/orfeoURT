...
//$this->cursor->debug=true;
    ...
    function radicar_salida_masiva($tipoanexo,$cuentai,$documento_us3 ,$med ,$fec,$radicadopadre,$codusuario,$tip_doc ,$ane,$pais,$asu,$coddepe,$tip_rem,$numdoc,$tdoc,$muni_codi,$archivo,$usua_doc,$depe_codi_territorial,$secRadicacion,$numeroExpediente='', $nurad, $fecSec)
    {
    	...
    	if ($sec!=-1) {
    		...
    		radi_usua_radi, tdid_codi, radi_desc_anex, radi_nume_hoja, radi_pais, ra_asun,
    		radi_depe_radi,	radi_usua_actu, carp_codi, radi_nume_radi, trte_codi, radi_nume_iden,
    		radi_fech_radi,	radi_depe_actu, tdoc_codi, esta_codi, ID_CONT, dpto_codi,
    		muni_codi, radi_path, carp_per)
    		VALUES	($tipoanexo, '$cuentai', $documento_us3, $med, '$fecSec', $radicadopadre,
    		'$codusuario', '$tip_doc', '$ane', 0, '$pais', '$asu',
    		'$coddepe', '$codusuario', '$carp_codi', '$nurad', '$tip_rem', '$numdoc',
    		'$fecSec', '$coddepe', '$tdoc', '9', $cont_codi, $dpto_codi,
    		$muni_codi, '$archivo', $carp_per)";
    ...
    function grabar_usuario($no_documento, $nombre_us1, $direccion_us1, $prim_apell_us1, $seg_apell_us1, $telefono_us1, $mail_nus1, $codep_us, $muni_us) {
        ...
        $codigo_con = $codigoMun[0];
        $codigo_pai = $codigoMun[1];
        $codigo_dep = $codigoMun[2];
        $codigo_mun = $codigoMun[3];

        $query = "INSERT INTO SGD_CIU_CIUDADANO( SGD_CIU_CEDULA, TDID_CODI, SGD_CIU_CODIGO, SGD_CIU_NOMBRE, SGD_CIU_DIRECCION, SGD_CIU_APELL1, " .
                "SGD_CIU_APELL2, SGD_CIU_TELEFONO, SGD_CIU_EMAIL, ID_CONT, ID_PAIS, DPTO_CODI, MUNI_CODI) values (" .
                "'$no_documento', 2, $nextval, '$nombre_us1', '$direccion_us1', '$prim_apell_us1', '$seg_apell_us1', '$telefono_us1', '$mail_nus1', '$codigo_con','$codigo_pai', '$codigo_dep', '$codigo_mun')";
        $rs = $this->cursor->query($query);
    ...
    function grabar_oem($no_documento, $nombre_us1, $direccion_us1, $prim_apell_us1, $seg_apell_us1, $telefono_us1, $mail_nus1, $codep_us, $muni_us) {
        ...
        switch (count($codigoMun)) {
            ...
            case 2: {
                    $codigo_con = 1;
                    $codigo_pai = 170;
                    $codigo_dep = $codigoMun[0];
                    $codigo_mun = $codigoMun[1];
                }break;
            case 3: {
                    $codigo_con = 1;
        ...
        $query = "INSERT INTO SGD_OEM_OEMPRESAS (tdid_codi, SGD_OEM_CODIGO, SGD_OEM_NIT, SGD_OEM_OEMPRESA, SGD_OEM_DIRECCION, SGD_OEM_REP_LEGAL,".
				"SGD_OEM_TELEFONO, ID_CONT, ID_PAIS, DPTO_CODI, MUNI_CODI, EMAIL) values " .
                "(4, $nextval, '$no_documento', '$nombre_us1', '$direccion_us1', '$prim_apell_us1', '$telefono_us1', ".
				"$codigo_con, $codigo_pai, $codigo_dep, $codigo_mun, '$mail_nus1')";
        $rs = $this->cursor->query($query);

        if (!$rs) {
            $this->cursor->conn->RollbackTrans();
            die("<span class='etextomenu'>No se ha podido insertar la informaci&oacute;n de OEM ($query)");
        }
        return $nextval;
    }

    function grabar_dir($tipo_rem, $nombre, $prim_apell, $seg_apell, $dignatario, $direccion, $depto, $muni, $radicado, $cod_usuario, $mail_us1) {
