...
class  Mensaje {
   ...
   */
	...
	* @param string	$usrDoc	Documento del usuario
	* @param integer	$Dependencia	$Dependencia a la que pertenece el usuario
	...
	*/
	function getMsgsUsr($usrDoc,$Dependencia){
		...
		$sql = "select m.SGD_MSG_CODI,
                        m.SGD_MSG_URL,
                        m.SGD_MSG_VECES,
                        t.SGD_TME_CODI,
                        m.SGD_MSG_ANCHO,
                        m.SGD_MSG_LARGO 
                    from  SGD_MSDEP_MSGDEP md,
                        USUARIO u,
                        SGD_USD_USUADEPE USD,
                        SGD_MSG_MENSAJE m,
                        SGD_TME_TIPMEN t
                    where t.SGD_TME_CODI = m.SGD_TME_CODI and 
        				u.USUA_DOC = '$usrDoc' and
                        USD.DEPE_CODI = $Dependencia and
                        U.USUA_DOC = USD.USUA_DOC AND
                        U.USUA_LOGIN = USD.USUA_LOGIN AND
                        USD.SGD_USD_SESSACT = 1 AND
		        		md.DEPE_CODI = USD.DEPE_CODI and
                        m.SGD_MSG_FECHDESP <= $sqlFechaHoy and
