...
if ($_POST['tipo'] == 1) {    //SOLITUD DE CAMBIO DE TRD
    if ($_POST['rad'] != null) {
        ...
        $fyh = BODEGAPATH . "logsTRD\\" . "SOLICITUDCAMBIO_" . $_POST['rad'] . "_" . date('Ymd_His') . ".txt";
        //debug($fyh, "inicio script" . "\n");
        
        //debug($fyh, "SOLICITUD DE CAMBIO DE TRD: $notifica - $seractu - $subseractu - $tdocactu - $serie - $tsub - $tdoc - $depe - $depesel \n");
        ...
        if ($rsSer && !$rsSer->EOF) {
            $nomser = iconv('iso-8859-1', 'utf-8', $rsSer->fields['SGD_SRD_DESCRIP']);
        }
        $rsSSer = $db->conn->Execute($isqlSSer);
        if ($rsSSer && !$rsSSer->EOF) {
            $nomsubser = iconv('iso-8859-1', 'utf-8', $rsSSer->fields['SGD_SBRD_DESCRIP']);
        }
        $rsTdoc = $db->conn->Execute($isqlTdoc);
        if ($rsTdoc && !$rsTdoc->EOF) {
            $nomtipo = iconv('iso-8859-1', 'utf-8', $rsTdoc->fields['SGD_TPR_DESCRIP']);
        }
        ...
        if ($rsSera && !$rsSera->EOF) {
            $nomsera = utf8_encode($rsSera->fields['SGD_SRD_DESCRIP']);
        }
        $rsSSera = $db->conn->Execute($isqlSSera);
        if ($rsSSera && !$rsSSera->EOF) {
            $nomsubsera = utf8_encode($rsSSera->fields['SGD_SBRD_DESCRIP']);
        }
        $rsTdoca = $db->conn->Execute($isqlTdoca);
        if ($rsTdoca && !$rsTdoca->EOF) {
            $nomtipoa = utf8_encode($rsTdoca->fields['SGD_TPR_DESCRIP']);
        }
        ...
        if ($rsus && ! $rsus->EOF) {
            while (! $rsus->EOF) {
                ...
                if ($emailUSU && $emailUSU != '') {
                    $asunto = "Envío de notificación radicado ".$_POST['rad'];
            ...
            //debug($fyh, "ENVIO CORREO: $ok -  $sqlUpd \n");
            ...
            {        
                ...
                //debug($fyh, "inserta historico: $radHist \n");
            }
        }
    }
    echo $result;
...
elseif ($_POST['tipo'] == 2) {    // APROBACION SI O NO DE CAMBIO DE TRD
    if ($_POST['rad'] != null) {
        ...
        $fyh = BODEGAPATH . "logsTRD\\" . "APROBACIONCAMBIO_" . $_POST['rad'] . "_" . date('Ymd_His') . ".txt";
        //debug($fyh, "inicio script" . "\n");
        
        //debug($fyh, "APROBACION DE CAMBIO DE TRD: $notifica - $aprueba \n");
        ...
        if ($ok) {
            ...
            //debug($fyh, "INSERTAR HISTORICO: $radHist \n");
        }
    ...
    if ($_POST['serie'] != null) {
        ...
        $querySub = "SELECT	distinct (".$db->conn->Concat("convert(char(4),su.sgd_sbrd_codigo,0)","'-'","su.sgd_sbrd_descrip").") as detalle, su.sgd_sbrd_codigo
                   ...
                   WHERE	m.depe_codi = ".$_SESSION['dependencia']." and
                            ...
                            '$fecha_hoy' between su.sgd_sbrd_fechini and
                            su.sgd_sbrd_fechfin
                   ORDER BY detalle";
...
elseif ($_POST['tipo'] == 5) {   //modificar TRD
    if ($_POST['rad'] != null) {
        ...
        $fyh = BODEGAPATH . "logsTRD\\" . "MODIFICARTRD_" . $nurad . "_" . date('Ymd_His') . ".txt";
        //debug($fyh, "inicio script" . "\n");
        
        //debug($fyh, "modificar TRD: $just - $nurad - $dependencia - $codusuario - $usua_doc - $tdoc - $codserie - $tsub - $cambio \n");
                ...
                if (isset($padre)) {
                    ...
                    if ($rsPadre && !$rsPadre->EOF) {
                        ...
                        if (date("Y-m-d") < $format_date && !isset($prorroga)) {
                            ...
                            $ok = $db->conn->Execute($sqlUpd);
                            //debug($fyh, "FECHA VENCIMIENTO: $ok - $sqlUpd \n");
        ...
        if ($rsDep && !$rsDep->EOF) {
            ...
            //debug($fyh, "ACTUALIZAR TRD: $rsUp - $sqlUA \n");
        } else {
        ...
        //debug($fyh, "HISTORICO CAMBIO: $ejecutarQuerey - $queryGrabar \n");
...
elseif ($_POST['tipo'] == 7) {    //   ELIMINAR TRD
    if ($_POST['rad'] != null) {
        ...
        $fyh = BODEGAPATH . "logsTRD\\" . "ELIMINARTRD_" . $nurad . "_" . date('Ymd_His') . ".txt";
        debug($fyh, "inicio script" . "\n");
        
        debug($fyh, "ELIMINAR TRD: $nurad - $coditrd - $dependencia - $codusuario - $usua_doc \n");
        ...
        if ($respuesta == 1) {
            ...
            if ($radiModi)
                ...
                $respuesta = 0;
            debug($fyh, "INSERTAR HISTORICO: $radiModi \n");
            ...
            debug($fyh, "ELIMINAR TRD: $respuesta \n");
...
elseif ($_POST['tipo'] == 8) {     //  INSERTAR TRD
    if ($_POST['rad'] != null) {
        ...
        $fyh = BODEGAPATH . "logsTRD\\" . "INSERTARTRD_" . $nurad . "_" . date('Ymd_His') . ".txt";
        debug($fyh, "inicio script" . "\n");
        
        debug($fyh, "INSERTAR TRD: $nurad - $dependencia - $codusuario - $usua_doc - $tipodoc - $codserie - $subserie \n");
        ...
        if ($res_coditrdx && !$res_coditrdx->EOF) {
            $TDCcactu = $res_coditrdx->fields['SGD_TPR_CODIGO'];
        ...
        if ($respuesta == '1') {
            ...
            if ($respuesta == '0') {
                ...
                if ($respuesta == 1) {
                    ...
                    debug($fyh, "INSERTA TRD: $radicados \n");
                    ...
                    if ($respuesta == '1') {
                        ...
                        $isqlSer = "SELECT SGD_SRD_DESCRIP FROM SGD_SRD_SERIESRD WHERE SGD_SRD_CODIGO = $codserie ";
                        $isqlSSer = "SELECT SGD_SBRD_DESCRIP FROM SGD_SBRD_SUBSERIERD WHERE SGD_SBRD_CODIGO = $subserie AND SGD_SRD_CODIGO = $codserie ";
                        $isqlTdoc = "SELECT SGD_TPR_DESCRIP FROM SGD_TPR_TPDCUMENTO WHERE SGD_TPR_CODIGO = $tipodoc ";
                        $rsSer = $db->conn->Execute($isqlSer);
                        if ($rsSer && !$rsSer->EOF) {
                        ...
                        if ($TDCcactu == 0) {
                            $observa = "Nuevo tipo documental: " . $nomtipo;
                        } else {
                            $observa = "Tipo documental anterior: " . $TDCactu . " - Nuevo: " . "$nomser/$nomsubser/$nomtipo";
                        }
                        
                        $radiModi = $Historico->insertarHistorico($codiRegH, $dependencia, $codusuario, $dependencia, $codusuario, $observa, $transac);
                        debug($fyh, "INSERTA historico: $radiModi - $observa \n");
    				    ...
    				    debug($fyh, "ACTUALIZAR TRD: $radiUp - $tipodoc \n");
...
elseif ($_POST['tipo'] == 9) {     //  INSERTAR TRD
    if ($_POST['nurad'] != null) {
        
        $ok = false;
        $sqlDt = "SELECT caux.SGD_CAUX_CODIGO,cau.SGD_CAU_DESCRIP as SECTOR, dcau.SGD_DCAU_DESCRIP as CAUSAL,caux.SGD_CAUX_CODIGO,caux.RADI_NUME_RADI COUNT_RADI, caux.SGD_CAUX_FECHA, dcau.SGD_DCAU_CODIGO
        	 FROM SGD_CAUX_CAUSALES caux, SGD_CAU_CAUSAL cau, SGD_DCAU_CAUSAL dcau
        	 WHERE RADI_NUME_RADI = ". $_POST['nurad'] ." and caux.SGD_DCAU_CODIGO=dcau.sgd_dcau_codigo and dcau.SGD_CAU_CODIGO=cau.SGD_CAU_CODIGO ";
        $rsDt = $db->conn->Execute($sqlDt);
        if ($rsDt && ! $rsDt->EOF) {
            
            $sqlUpd = "UPDATE SGD_CAUX_CAUSALES SET SGD_DCAU_CODIGO = '".$_POST['tema']."', SGD_CAUX_FECHA = ".$db->conn->OffsetDate(0, $db->conn->sysTimeStamp)."
                WHERE RADI_NUME_RADI = " . $_POST['nurad'];
            $ok = $db->conn->Execute($sqlUpd);
            
        } else {
            
            $sql = "select max(SGD_CAUX_CODIGO)+1 as max from SGD_CAUX_CAUSALES";
            $rs = $db->conn->Execute($sql);
            if ($rs && ! $rs->EOF) {
                $sqlUpd = "INSERT INTO SGD_CAUX_CAUSALES (SGD_DCAU_CODIGO, SGD_CAUX_FECHA, RADI_NUME_RADI, SGD_CAUX_CODIGO)
                        VALUES ('".$_POST['tema']."', ".$db->conn->OffsetDate(0, $db->conn->sysTimeStamp).", ".$_POST['nurad'].", ".$rs->fields['MAX'].")";
                $ok = $db->conn->Execute($sqlUpd);
            }
