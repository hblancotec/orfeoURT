...
/**
 * Esp es la clase encargada de gestionar las operaciones y los datos básicos referentes a una Esp
...
/**
 * Genera un archivo CSV con la información de las empresas que recibe como parámetro
...
{ 	if (${'s'.$i})		//Si se envio el vector....
	{
		...
		//variable que almacena la cantidad de empresas que llegarom como parámetro
		$num = count($arrEmpresas);
		$x = 0;

		//Se recorre el arreglo de las empresas para armar el IN dentro del where de la búsqueda
		while ($x < $num)
		{
			$nuir=$arrEmpresas[$x];
		...
		//query final
	  	switch ($i)
	  	{	case 0:
	  			{	$q= "select ID_PAIS AS IDPAIS,CODIGO_DEL_DEPARTAMENTO AS IDDPTO,CODIGO_DEL_MUNICIPIO AS IDMPIO,
	  						NOMBRE_DE_LA_EMPRESA AS PPAL,DIRECCION AS DIR,NUIR AS ID,NOMBRE_REP_LEGAL AS DESCR
	  					from BODEGA_EMPRESAS where NUIR in ($selectedSQL) ORDER BY CODIGO_DEL_DEPARTAMENTO, CODIGO_DEL_MUNICIPIO, NOMBRE_DE_LA_EMPRESA";
	  			}break;
	  		case 1:
...
/**
 * Genera un archivo PDF con la información de las empresas que recibe como parámetro
 * @param	$s(0/1/2/)	string	con las (ESP/Empresas/Ciudadanos) seleccionadas
