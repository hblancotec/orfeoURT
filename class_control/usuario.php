...
class Usuario
{
    ...
    function usuarioDependecina($dependencia, $codUsuario)
    {
        $q = "select *,
                usuario.USUA_NOMB,
                    ...
                    usuario.USUA_EMAIL,
                    usuario.USUA_NACIM
				 from usuario, SGD_USD_USUADEPE USD
                where USD.depe_codi = $dependencia and
                    usuario.USUA_DOC = USD.USUA_DOC AND
                    usuario.USUA_LOGIN = USD.USUA_LOGIN AND
    ...
    /**
     * Genera el javascript que ha de permitir llenar un combo con los usuarios de cierta dependencia de un grupo de dependencias establecidas por el paramentro enviado a la funcion
     *
     * @param $dependencias integer. Es el grupo de dependencias establecido
     * @return void
     */
    ...
    /**
     * Genera el javascript que ha de permitir llenar un combo con los usuarios de cierta dependencia de un grupo de dependencias establecidas por el paramentro enviado a la funcion, haciendo uso del par�metrop where que llega y que filtra ciera caracter�atica del usuario a seleccioner
     *
     * @param $dependencias integer. Es el grupo de dependencias establecido
     * @param $where string. Es el criterio de filtro para los usuarios de las dependencias seleccionadas
     * @return void
     */
    ...
    /**
     * Retorna el valor del login.
     *
     * @return string
     */
    function get_usua_login()
    {
        return $this->usua_login;
    }
    
    /**
     * Retorna el valor del correo electronico.
     *
     * @return string
     */
    function get_usua_mail()
    {
     ...
     *
     * @param $docto string. Es el documento del usuario
     * @return void
     */
    ...
    /**
     * Carga los datos de la instacia con con referencia a un documento de usuario suministrado
     *
     * @param $docto string. Es el documento del usuario
     * @return void
     */
    function usuarioIdDepto($Id, $IdDepe)
    {
        global $ADODB_COUNTRECS;
        $ADODB_COUNTRECS = true;
        $q = "select * from usuario where USUA_CODI = $Id and DEPE_CODI = $IdDepe";
        $rs = $this->cursor->query($q);
        $ADODB_COUNTRECS = false;
        if ($rs->RecordCount() > 0) {
            $this->depe_codi = $rs->fields['DEPE_CODI'];
