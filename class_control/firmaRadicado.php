  ...
  /** 
	...
	* @return   boolean
	...
	function obtenerCicloPorRadicado($radicado){
	    $sql = "select top 1 m.estado, m.idcf, m.rutapdf as ruta, r.radi_path ".
	           "from sgd_ciclofirmadomaster m inner join radicado r on r.radi_nume_radi=m.radi_nume_radi ".
	           "where m.radi_nume_radi=$radicado order by m.sgd_ciclo_fechasol desc";
	            ...
	            case 'docx': {
	            	if (file_exists(BODEGAPATH.$ruta)) {
	                	if (copy(BODEGAPATH.$ruta, BODEGAPATH.'dav/'.date('Y').'/'.$info['filename'].'.'.$info['extension'])){
	                	    try {
    	                        $sql = "insert into sgd_ciclofirmadomaster (radi_nume_radi,usua_login,usua_doc, estado, rutapdf) values ($radicado, '".$_SESSION['login']."', '".$_SESSION['usua_doc']."', 1, 'dav/".date('Y').'/'.$info['filename'].'.'.$info['extension']."')";
	        ...
	        if ($rs->RecordCount() == 0) {
	            ...
	            if (($opcMod == 2) || ($opcMod == 1) )  {
    	            ...
    	            if (is_integer($esSolicitante)) {
    	                ...
    	                $sql = "insert into sgd_hist_ciclofirmado (idcf, usua_login, usua_doc, sgd_ttr_codigo, codigo_rechazo, detalle) values ($ciclo, '".$login."', '".$cedula."', $ttr, $opcMod, ".$this->cursor->conn->qstr($obs, get_magic_quotes_gpc()).")";
    	                $this->cursor->conn->Execute($sql);
    	            } else {
