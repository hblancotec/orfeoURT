<?php
	...
	include (ORFEOPATH . "include/query/queryver_datosrad.php");

	$isql = "SELECT	A.*, 
					$numero, 
					$radi_nume_radi as RADI_NUME_RADI,
                    $radi_nume_deri as RADI_NUME_DERI,
                    A.SGD_SPUB_CODIGO NIVEL_SEGURIDAD
			FROM	RADICADO A
    ...
    $mrec_codi      = $rs->fields["MREC_CODI"];
    $despla         = $rs->fields["RADI_DESPLA"];
	$radi_nume_soli = $rs->fields["RADI_NUME_SOLICITUD"];
	...
	$radi_exp_solicitudes = $rs->fields["RADI_USUA_RADI"];

	$isqlx = "	SELECT	EXP_SOLI_TIPO
				FROM	RADICADO R, SGD_EXP_SOLICITUDES S
				WHERE	S.EXP_SOLI_ID = R.EXP_SOLI_ID AND
						R.RADI_NUME_RADI = $verradicado";
	
	$rsx 	   = $db->conn->Execute($isqlx);
    $soli_tipo = $rsx->fields["EXP_SOLI_TIPO"];
	...
	$radi_respuesta = $rs->fields["RADI_RESPUESTA"];
	$nivelRad       = $rs->fields["NIVEL_SEGURIDAD"];

	// El nivel de seguridad basico viene del radicado,
    // pero si el Expediente en el que se encuentra tiene seguridad diferente de publico
	// Este determina el verdadero nivel de seguridad del radicado
	if( $perm == 1 )
		$nivelRad = 1;
	$sgd_id_tram	= $rs->fields["SGD_ID_TRAM"];
	...
	$tpdoc_rad      = $rs->fields["TDOC_CODI"];
        $sgd_tema       = $rs->fields["RADI_TEMA_ID"];
	//HLP $sgd_tres_codigo = $rs->fields["SGD_TRES_CODIGO"];
	//HLP $fechaNotific = $rs->fields["RADI_FECH_NOTIF"];
	$sgd_apli_codi  =  $rs->fields["SGD_APLI_CODI"];
	if ($rs->fields["RADI_PATH"]){
		$imagenv = "<a  class='vinculos' href='bodega/" .
				$rs->fields["RADI_PATH"] . "' target='$fechah'>Ver Imagen en Otra Ventana</a> ";
		} else {
		...
		//$radicadopadre = $radi_nume_deri; //jcr
		include (ORFEOPATH . "radicacion/busca_direcciones.php");
    ...
    //Nov2 fin
     ...
     //Nov 2 ini
      //Otro Destinatario
      $nombret_us7_u = trim($nombret_us7);
      //Nov2 fin
    ...
    //Nov 2 ini
    //Otro Destinatario
    if($tipo_emp_us7>0){$nombret_us7_u = trim($nombre_us7);}
     //Nov2 fin
    ...
    if($carpeta==8) {
     ...
     $datos_envio="&otro_us11=$otro_us1&dpto_nombre_us11=$dpto_nombre_us1&muni_nombre_us11=$muni_nombre_us1&direccion_us11=$direccion_us1&codpostal_us11=$codpostal_us1&nombret_us11=$nombret_us1";
     $datos_envio.="&otro_us2=$otro_us2&dpto_nombre_us2=$dpto_nombre_us2&muni_nombre_us2=$muni_nombre_us2&direccion_us2=$direccion_us2&codpostal_us2=$codpostal_us2&nombret_us2=$nombret_us2";
     $datos_envio.="&dpto_nombre_us3=$dpto_nombre_us3&muni_nombre_us3=$muni_nombre_us3&direccion_us3=$direccion_us3&codpostal_us3=$codpostal_us3&nombret_us3=$nombret_us3";

    //Nov 2 ini
    //Otro Destinatario
    $datos_envio.="&dpto_nombre_us7=$dpto_nombre_us7&muni_nombre_us7=$muni_nombre_us7&direccion_us7=$direccion_us7&codpostal_us7=$codpostal_us7&nombret_us7=$nombret_us7";
    //Nov2 fin
...
if($sgd_id_tram)
{
	$isql = "select SGD_NOMBR_TRAM FROM SGD_TRAMITES where SGD_ID_TRAM=$sgd_id_tram ";
	$rs=$db->conn->Execute($isql);
	if  (!$rs->EOF)
		$tramite_nombre = $rs->fields["SGD_NOMBR_TRAM"];
}
if($sector_grb)
{
