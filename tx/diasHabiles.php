...
function diasHabiles($fechaIni,$fechaFin) {
	...
	$isql_busca = "	SELECT COUNT(*) DIASH
			from sgd_noh_nohabiles
			where " . $this->db->conn->SQLDate('Y/m/d', 'noh_fecha') . " >= '$fechaIni' AND
			" . $this->db->conn->SQLDate('Y/m/d', 'noh_fecha') . " <= '$fechaFin'";
	$rs = $this->db->query($isql_busca);
	$diasFestivos = 0;
	if(!$rs->EOF ) {
		 $diasFestivos =  $rs->fields["DIASH"];
	}
    $fechaArreglo = explode(' ', $fechaIni);
    $fechaIni = $fechaArreglo[0];
	$fechaTMP = $fechaIni;
    $ii = 0;
    $diasFinSemana = 0;
    $kDia = 0;
