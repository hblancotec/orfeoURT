	...
	$query="select *
	  ...
	  if(!$rs->fields["SGD_EXP_NUMERO"])
	  {
	    $this->db->conn->Execute($query);
			...
			$rs2 = $this->db->conn->Execute($query);
			if(!$rs2->fields["SGD_EXP_NUMERO"])
			{
					...
					"<hr> $num_expediente,$radicado,$depe_codi,$usua_codi,$usua_doc,$usua_doc,$codiSRD,$codiSBRD,"."true".",$fechaExp <hr>";
...
/**
  ...
  */
	...
	* @param  $nomb_Expe_300  String Con tamaño de 300 caracteres. Nombre asignado al expediente
	* @return 1 si la consulta se ejecuto con exito
	*/

	function insert_ExpedienteNomb($num_expediente,$nomb_Expe_300){
	 	$sqlUpdate = "	UPDATE 
							SGD_SEXP_SECEXPEDIENTES SET SGD_SEXP_PAREXP5 = '$nomb_Expe_300' 
						WHERE 
							SGD_EXP_NUMERO = '$num_expediente'";			
		return $db->conn->Execute($sqlUpdate);
	 }
