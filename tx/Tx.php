...
function registrarNovedad($tipo, $docUsuarioDest, $numRad, $ruta_raiz=""){
	...
	if($param->PARAM_VALOR=="1"){ 
		...
		$rs=$this->db->conn->Execute($qs);
	        
		if($rs->fields['CONTADOR'] == 0){
					$qu="INSERT INTO SGD_NOVEDAD_USUARIO (USUA_DOC,$tipo1) values ($docUsuarioDest,$valor)";
					$this->db->conn->Execute($qu);
			
			}else{
					$this->db->conn->Execute("UPDATE SGD_NOVEDAD_USUARIO SET $tipo1 = $valor where USUA_DOC'$docUsuarioDest'");				
				}
		...
		if($rs->fields['CONTADOR'] == 0){ 
					...
					$this->db->conn->Execute("UPDATE SGD_NOVEDAD_USUARIO SET $tipo1 = $valor where $campo='$docUsuarioDest'");				
				}
