...
if (!$_SESSION['dependencia'] or !$_SESSION['depe_codi_territorial'] or !$_SESSION['depe_municipio'] )  include "$ruta_raiz/rec_session.php";
...
while ($i < $num) {	
  ...
  fputs ($fp,"*DEP_SIGLA*=$dep_sigla\n");
	...
	for ($i_count=0;$i_count<count ($campos);$i_count++){
		...
		//print ("<BR> añade".trim($campos[$i_count])."=".trim($datos[$i_count])."\n");
	}
	fclose($fp);
