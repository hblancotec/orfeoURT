<?php
    ...
    if( isset( $_POST['grabar'] ) && $_POST['grabar'] == "GRABAR" ) {
        ...
        $grabarSubexpediente = $expediente->grabarSubexpediente( $_GET['nurad'],
                                                                    $_GET['num_expediente'],
                                                                    $_POST['exp_subexpediente'],
                                                                    $_POST['exp_nombresubexp']);
        if( $grabarSubexpediente == 1 ) {
            $observa = "Creaci&oacute;n de Subexpediente";
            include_once "$ruta_raiz/include/tx/Historico.php";
            $radicados[] = $_GET['nurad'];
            $tipoTx = 55;
            $Historico = new Historico( $db );
            $Historico->insertarHistoricoExp($_GET['num_expediente'],
                                                $radicados,
                                                $dependencia,
                                                $codusuario,
                                                $observa,
...
<?php
            ...
            echo '<hr><font color="red">No se guard&oacute; el Subexpediente. Por favor intente de nuevo.</font><hr>';	    
        }
    }
?>
<html>
...
?>
    ...
    function grabarSubexpediente() {
        if( document.getElementById( 'exp_subexpediente').value == "" ||
                isNaN( document.getElementById( 'exp_subexpediente').value ) ) {
            alert( "Debe ingresar un numero de subexpediente." );
            document.getElementById( 'exp_subexpediente').focus();
        } else {
            confirmar = confirm( "Recuerde que cualquier cambio que realice en este modulo afectaria su \n\r ubicacion fisica dentro del expediente." );
        }
...
<?php
				...
				if( $Grabar ) {
                    ...
                    $exp_nombresubexp = $_POST['exp_nombresubexp'];
                    $exp_subexpediente = $_POST['exp_subexpediente'];
                    $btt->modificar_expediente($nurad,
                                                $num_expediente,
                                                $exp_titulo,
                                                $exp_asunto,
                                                $exp_ufisica,
                ...
                $nombreSubexp = $btt->exp_nombresubexp;
?>
				</tr>
              </table>
            </td>
          ...
          <tr> 
            <td class="listado2"> 
              <table width="100%" height="99%" cellspacing="5"  align="center" class="borde_tab" >
                ...
                <tr>
                  <td class='titulos2'>NOMBRE SUBEXPEDIENTE</td>
			      <td class='titulos2' nowrap>
                    <input type="text" class="tex_area" name="exp_nombresubexp" id="exp_subexpediente" value='<?=$nombreSubexp?>' size="50" maxlength="255">
                    &nbsp;
                  </td>
				</tr>
				<tr class='titulos2'>
