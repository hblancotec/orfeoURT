...
{
    $mensaje  = "<center class='titulosError2'>ESTÁ SEGURO DE INCLUIR EL(LOS) RADICADO(S) EN EL(LOS) EXPEDIENTE(S):</center> <br>";
    ...
    $mensaje .= " en Archivo crean una carpeta f&iacute;sica en la cual empezarán a incluir los";
    ...
    if( isset( $_POST['confirmaIncluirExp'] ) && $_POST['confirmaIncluirExp'] == "INCLUIR_EXP" )
    {
        ...
        foreach( $arrExpSeleccionados as $clave => $numExpediente )
        {
            foreach( $arrRadSeleccionados as $llave => $numRadicado )
            {
                ...
                {
                    print '<hr><font color=red>No se incluyó el radicado No. '.$numRadicado.' en el expediente No. '.$numExpediente.'. Por favor intente de nuevo.</font><hr>';
                    break;
                }
            }
        }
