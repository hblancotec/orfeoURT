<?php
    ...
    $encabezado = $_SERVER['PHP_SELF']."?".session_name()."=".session_id()."&opcionExp=$opcionExp&numeroExpediente=$numeroExpediente&dependencia=$dependencia&krd=$krd&nurad=$nurad&coddepe=$coddepe&codusua=$codusua&depende=$depende&ent=$ent&tdoc=$tdoc&codiTRDModi=$codiTRDModi&codiTRDEli=$codiTRDEli&codserie=$codserie&tsub=$tsub&ind_ProcAnex=$ind_ProcAnex";

    include_once ("$ruta_raiz/include/tx/Expediente.php");
    $expediente = new Expediente( $db );

    //Dependencia para no mostrar los expedietes que no pertenecen a esta
    //estos no se podran excluir
    ...
    if( isset( $_POST['expSeleccionados'] ) && $_POST['expSeleccionados'] != "" ) {
        ...
        foreach( $arrExpSeleccionados as $clave => $numExpediente ) {
            ...
            // Si el radicado no esta archivado
                ...
                $mensaje = "Va a excluir este documento del(os) Expediente(s) seleccionado(s). <br> Esta seguro?";
            }
        }
        ...
        if( isset( $_POST['confirmaIncluirExp'] ) && $_POST['confirmaIncluirExp'] == "EXCLUIR_EXP" ) {
            foreach( $arrExpSeleccionados as $clave => $numExpediente ) {
                ...
                if( $resultadoExp == 1 ) {
                    ...
                    print '<hr><font color=red>No se excluy� este radicado del expediente No. '.$numExpediente.'. Por favor intente de nuevo.</font><hr>';
                    break;
                }
            }
        ?>
...
?>
    <tr class="listado1">
      ...
      <?php
      	$depeExp	=  substr($numeroExpediente,4,4);				
		if($depeExp == $depeCodi)
		{
      	  ?>
	      <td align="center">
          ...
          <?php
			...
			echo '<td colspan="2" align="center"> No pertenece a esta dependencia, no puede excluirlo.</td>';
		}
      ?>
    </tr>
<?php
