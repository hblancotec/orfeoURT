<?php
    ...
    // Consulta los parametros creados para la dependencia
    $sqlBuscarPor  = "SELECT SGD_PAREXP_CODIGO,
                                SGD_PAREXP_ETIQUETA,
                                SGD_PAREXP_ORDEN";
    $sqlBuscarPor .= " FROM SGD_PAREXP_PARAMEXPEDIENTE PE";
    $sqlBuscarPor .= " WHERE PE.DEPE_CODI = ( ";
    $sqlBuscarPor .= " SELECT USD.DEPE_CODI";
    $sqlBuscarPor .= " FROM USUARIO U,
                            SGD_USD_USUADEPE USD 
                        WHERE U.USUA_LOGIN = '".$krd."' AND
                                U.USUA_LOGIN = USD.USUA_LOGIN AND
                                U.USUA_DOC = USD.USUA_DOC AND
                                USD.SGD_USD_SESSACT = 1";
    $sqlBuscarPor .= " )";
    $rsBuscarPor = $db->conn->Execute( $sqlBuscarPor );
...
<table border=0 width="78%" class="borde_tab" cellpadding="0" cellspacing="5">
    ...
    <?php
        foreach ( $arrComboBuscarPor as $idBuscarPor => $valorBuscarPor ) {
            echo "<option value='$idBuscarPor'>$valorBuscarPor</option>";
        }
    ?>
            </select>
		  </font>
...
<?php
	...
	switch ($driver) {
		case 'oci8':
			...
			break;
		case 'mssqlnative':
			$q_select  = "SELECT CASE";
			$q_select .= "    WHEN SGD_CAMEXP_FK = 0 THEN SGD_CAMEXP_CAMPO";
			$q_select .= "    WHEN SGD_CAMEXP_FK = 1 THEN SGD_CAMEXP_CAMPOVALOR";
			$q_select .= " END";
			$q_select .= " AS CAMPO,";
			$q_select .= " CASE";
			$q_select .= "    WHEN SGD_CAMEXP_FK = 1 THEN PE.SGD_PAREXP_TABLA" .$db->conn->concat_operator. "'.'" .$db->conn->concat_operator. "CE.SGD_CAMEXP_CAMPO" .$db->conn->concat_operator. "' = '" .$db->conn->concat_operator. "CE.SGD_CAMEXP_TABLAFK" .$db->conn->concat_operator. "'.'" .$db->conn->concat_operator. "CE.SGD_CAMEXP_CAMPOFK";
			$q_select .= " END";
			$q_select .= " AS FILTRO,";
			$q_select .= " CASE";
			$q_select .= "    WHEN SGD_CAMEXP_FK = 0 THEN PE.SGD_PAREXP_TABLA";
			$q_select .= "    WHEN SGD_CAMEXP_FK = 1 THEN CE.SGD_CAMEXP_TABLAFK";
			$q_select .= " END";
			$q_select .= " AS TABLA";
			$q_select .= " FROM SGD_CAMEXP_CAMPOEXPEDIENTE CE, SGD_PAREXP_PARAMEXPEDIENTE PE";
			$q_select .= " WHERE PE.SGD_PAREXP_CODIGO = CE.SGD_PAREXP_CODIGO";
			$q_select .= " AND CE.SGD_PAREXP_CODIGO = ".$tbusqueda;			
	}
        $rs_select = $db->conn->Execute( $q_select );
        $c = 0;
        $tmp_tabla = "";
        while( !$rs_select->EOF ) {
        ...
        if (is_array($arr_select['campo'])) {
            $q_buscar  = "SELECT ".implode( ", ", $arr_select['campo'] );
        }
        if (is_array($arr_select['tabla'])) {
            $q_buscar .= " FROM ".implode( ", ", $arr_select['tabla'] );
        }
        $q_buscar .= " WHERE 1 = 1";
        
        if (is_array($arr_select['filtro'])) {
        ...
        $q_buscar .= " )";
        $db->conn->SetFetchMode( ADODB_FETCH_NUM );
        $rsBuscar = $db->conn->Execute( $q_buscar );
...
?>
      <tr class='<?=$grilla ?>'>
        ...
        <td class="listado5">
          <font size="-3">
          <?php
            echo substr( $rsBuscar->fields[1], 0, 120 );
          ?>
          </font>
        </td>
        <td class="listado5">
          <font size="-3">
          <?php
            echo substr( $rsBuscar->fields[2], 0, 70 );
          ?>
          </font>
        </td>
        <td class="listado5">
