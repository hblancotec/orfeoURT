<?php
	...
	$ruta_raiz = "..";
	if(!$krd) $krd = $krdold;
    $coddepe = (!empty($_GET['coddepe'])) ? $_GET['coddepe'] : null;
    $coddepe = (!empty($_POST['coddepe'])) ? $_POST['coddepe'] : null;
	...
	function crearProcesoExpediente($numExpediente,
                                    $radicado,
                                    $depe_codi,
                                    $usua_codi,
                                    $usua_doc,
		...
		$consultaEtaInicial="SELECT SGD_FEXP_CODIGO,
                                    SGD_PEXP_DESCRIP
                                FROM SGD_FEXP_FLUJOEXPEDIENTES f INNER JOIN SGD_PEXP_PROCEXPEDIENTES p on 
                                        f.SGD_PEXP_CODIGO=p.SGD_PEXP_CODIGO
...
<?php
	...
	print $rsD->GetMenu2("codserie",
                            ...
                            false,
                            "",
                            "onChange='submit()' class='select' id='codserie'");
 ?>
	</td>
	</tr>
	<tr>
...
?>
 ...
 <tr>
  ...
  <?php
	 if(!$fechaExp) $fechaExp = date("d/m/Y");
  ?>
   var dateAvailable1 = new ctlSpiffyCalendarBox("dateAvailable1",
                                                    "TipoDocu",
                                                    "fechaExp",
                                                    "btnDate1",
                                                    "<?=$fechaExp?>",
...
<?php
	...
	print $rsUs->GetMenu2("usuaDocExp",
                            "$usuaDocExp",
                            "0:-- Seleccione --",
                            false,
                            "",
                            " class='select' onChange='submit()'");
?>
	</td>
</tr>
	<tr align="center">
	...
	<?php  if($tsub && $codserie && $usuaDocExp) {?>
			...
			<input name="coddepe" type="hidden" id="coddepe" value="<?=$coddepe?>">
	<?php } ?>
	</center></td>
	<td width="33%" class="listado2" height="25">
	<center>
        <input name="cerrar" type="button" class="botones_funcion" id="envia22" onClick="opener.regresar(); window.close();" value=" Cerrar "></center></TD>
	</tr>
</table>
</form>
</body>
...
<?php
    if(isset($_POST['CrearProceso'])){
       	$sal = crearProcesoExpediente($numeroExpediente,
                                        $nurad,
                                        $coddepe,
                                        $codusuario,
                                        $usua_doc,
