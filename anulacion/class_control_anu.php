   ...
   var $tabla_htmlA;
				...
				if ($sec!=-1){
					  ...
					  $sec = str_pad($sec,6,"0",STR_PAD_left);
						...
						if (!$rs){
							...
							die ("<span class='etextomenu'>No se ha podido insertar la informacion del nuevo radicado $nurad"); 
						}
										
				}else{
					$this->cursor->conn->RollbackTrans();
					die ("<span class='etextomenu'>No existe secuencia para la generacion del radicado (Territorial $depe_codi_territorial)");
				}
				return $nurad;
	 }
	 ...
	 function grabar_oem($no_documento,$nombre_us1,$direccion_us1,$prim_apell_us1,$seg_apell_us1,$telefono_us1,$mail_nus1,$codep_us,$muni_us)
	 {
	   $query = "select max(SGD_OEM_CODIGO) as MAXIMO  from SGD_OEM_OEMPRESAS ";
		 ...
		 if (!$rs){
				$this->cursor->conn->RollbackTrans();
				die ("<span class='etextomenu'>No se ha podido insertar la informacion de OEM ($query)"); 
			}												
			return $nextval;
	 }
				...
				foreach($this->campos_tabla as $campos_d)
				  {
					...
					if(!trim($jh_db->fields($campos_d))) $dato = "-"; else $dato=substr($jh_db->fields($campos_d),0,$width_max);
					$tabla_html .=  "<td  align=$align width=".$this->campos_width[$iii]." height=23>$dato</td>";
					echo "<td class='listado2' align=$align>".$dato."</td>";
					$iii++;
				}
			$tabla_html .=  "</tr>";
			echo "</tr>";
	...
	echo "</table>";
	return (!empty($nextval)) ? $nextval : ($numreginf == 0) ? NULL : $numreginf;
   }
}
