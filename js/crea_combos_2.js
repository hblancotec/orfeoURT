...
function cambiaUnParametro(parametros){
	var forma = cambiaUnParametro.arguments[0];
	var ob2mod = cambiaUnParametro.arguments[1];
	var id2mod = cambiaUnParametro.arguments[2];
	var tx2mod = cambiaUnParametro.arguments[3];
	oCntrl = document.getElementById(ob2mod);
	if (oCntrl.type == 'select-one')	{
		for(var i = 0; i < forma.elements.length; i++)
		{	// Realizamos procesos y creamos variables respecto al combo a modificar.
			if (forma.elements[i].name == ob2mod)
			{	// Capturamos el objeto a modificar y su posicion en el formulario.
				oCntrl = forma.elements[i];
				pos = i;
				prefijo = (oCntrl.name).substring(0, (oCntrl.name).length-1);
		...
		borra_combo(forma, pos);
		addOpt(oCntrl, 0, tx2mod, id2mod);
	}
	if (oCntrl.type == 'hidden') {
		oCntrl.value = id2mod;
	}
}

function getIdVjs(matriz, cod) {
    var returnKey = -1;
    matriz.forEach(function(subv, key) {
        if (subv['ID1'] == cod) {
           returnKey = key;
           return false; 
        }; 
...
/*
*	Funcion que elimina todo el contenido de un combo (select). Se le envia el formulario y la posicion del objeto.
*/
