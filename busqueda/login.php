...
//-------------------------------
      ...
      $db->conn->Execute("SELECT USUA_LOGIN, CODI_NIVEL FROM USUARIO WHERE USUA_LOGIN =" . tosql($sLogin, "Text") . " AND USUA_PASW=" . tosql($sPassword, "Text"));
...
function Form1_show()
{
  ...
  $sFormTitle = "Ingrese usuario y contraseña";
...
//-------------------------------
?>
      <tr><td class="FieldCaptionTD"><font class="FieldCaptionFONT">Identificación</font></td><td class="DataTD"><input type="text" name="Login" value="<?=tohtml(get_param("Login"))?>" maxlength="50"></td></tr>
...
//-------------------------------
    ...
    $db->conn->Execute("SELECT USUA_LOGIN FROM USUARIO WHERE USUA_LOGIN=". tosql($usuario,"Text"));
    $db->next_record();
?>
