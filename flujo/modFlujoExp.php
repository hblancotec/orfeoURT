...
if($grabarFlujo)
{
  ...
  if(!$ddca_causal) 
	...
	$estadoModificado = $objFlujo->cambioNodoExpediente($numeroExpediente,
								$numRad,
								$estadoNuevo,
								$aristaActual,
								0,
								"Cambio Manual. ($flujo_nombre)",$_GET['texp']);
	
	// Si el estado del flujo ha sido modificado entonces enviar correo
	if($estadoModificado){
		$sqlUsuario = "SELECT USUA_DOC, USUA_EMAIL
				FROM USUARIO
		...
		if(!$rsUsuario->EOF){
			$usuarioMail = $rsUsuario->fields["USUA_EMAIL"];
		}
	}
