...
<?
	if ($_GET ['codigo'] != NULL) {
		?>
<center><input size="100" onClick="ubicacion('ubicacion','<?php echo $_GET ['codigo']; ?>',1,'<?php echo $_GET['depeA']; ?>','cancelar');" value="cancelar" type="button" class="botones" /><?
	}
} else {
     if(!$depe_codi ){
        $depe_codi=$_GET['depeA'];
	...
	?> <center><input size="100" onClick="ubicacion('ubicacion','<?php echo $ubica?>',2,'<?php echo $depe_codi; ?>','modificar');"	value="Cambiar Ubicacion" type="button" class="botones" /> <input type="hidden"
	name="exp_entre" value="<?php echo $ubica?>"> <input type="hidden" name="exp_caja" value="<?
	echo $ubica?>">
     <?
	}
}
