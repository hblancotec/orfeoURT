...
//$db->debug = false;
extract($_GET, EXTR_SKIP);extract($_POST, EXTR_OVERWRITE);
$encabezadol = $_SERVER['PHP_SELF']."?".session_name()."=".session_id()."&codig=$codig&cod=$cod";
?>
<script>
function regresar(){
...
for($i=0;$i<$tm;$i++){
        ...
        if(!$rsi->EOF)$nomp[$cp]=$rsi->fields['SGD_EIT_NOMBRE'];
        $codigo=$codig;
        $sql="select * from sgd_eit_items where cast(sgd_eit_cod_padre as varchar(18)) like '$codig'";
        $rs=$db->conn->Execute($sql);
        while(!$rs->EOF){
        $nom[$c]=$rs->fields['SGD_EIT_NOMBRE'];
        $codi[$c]=$rs->fields['SGD_EIT_CODIGO'];
...
$tm3=$c;
for($i=$tm2;$i<$tm3;$i++){
$sqli="select sgd_eit_nombre from sgd_eit_items where cast(sgd_eit_codigo as varchar(18))='$codi[$i]'";
//$db->conn->debug=true;
$rsi=$db->conn->Execute($sqli);
if(!$rsi->EOF)$nomp[$cp]=$rsi->fields['SGD_EIT_NOMBRE'];
$codigo=$codi[$i];
$sql="select * from sgd_eit_items where cast(sgd_eit_cod_padre as varchar(18)) like '$codi[$i]'";
$rs=$db->conn->Execute($sql);
while(!$rs->EOF){
$nom[$c]=$rs->fields['SGD_EIT_NOMBRE'];
$codi[$c]=$rs->fields['SGD_EIT_CODIGO'];
