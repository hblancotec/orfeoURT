...
$db->debug=true;
$encabezadol = $_SERVER['PHP_SELF']."?".session_name()."=".session_id()."&dependencia=$dependencia&krd=$krd&tipo=$tipo&sel=$sel&ano=$ano";
static $sel;
$numInv=1;
...
if($pisocod!=0){
	...
	while(!$rs2->EOF){
		...
		if($tmp1[0]==$tmp){
				include("$ruta_raiz/include/query/archivo/queryInventario.php");
	    		$rs4=$db->conn->Execute($sqli4);
	    		$TEMP+=$rs4->fields['CO'];
			}

		include("$ruta_raiz/include/query/archivo/queryInventario.php");
		$rs3=$db->conn->Execute($sqliD);
		...
		while (!$rs3->EOF){
			...
			while (!$rs4->EOF){
				...
				if($tmp3[0]==$tmp){
					include("$ruta_raiz/include/query/archivo/queryInventario.php");
	    			$rs6=$db->conn->Execute($sqli1);
		    		$TEMP+=$rs6->fields['CO'];
				}
				if($tmp3[0]==$IT2){
					include("$ruta_raiz/include/query/archivo/queryInventario.php");
	    			$rs6=$db->conn->Execute($sqlil);
		    		$DE+=$rs6->fields['CO'];
				}
				include("$ruta_raiz/include/query/archivo/queryInventario.php");
				$rs5=$db->conn->Execute($sqliQ);
				while (!$rs5->EOF){
	...
	for($an=2005;$an<=Date('Y');$an++){
		$exp=$an;
		include("$ruta_raiz/include/query/archivo/queryInventario.php");
		if($min==1){$rs8=$db->conn->Execute($sqlo3);$rs9=$db->conn->Execute($sqlo4);}
		if($min==2){$rs8=$db->conn->Execute($sqlo2);$rs9=$db->conn->Execute($sqlo31);}
		if(!$rs8->EOF){
			$gpr=$rs8->fields['ITS'];
			$ctotal+=$gpr;
			}
...
<?php
	if($exp=='>>'){
	include("$ruta_raiz/include/query/archivo/queryInventario.php");
	$rsex=$db->conn->Execute($sqlex);
	$unidoc2=$rsex->fields['CARP'];
	$rsex2=$db->conn->Execute($sqlex2);
	if (!$rsex2->EOF){
...
include("$ruta_raiz/include/query/archivo/queryInventario.php");
if($rs=$db->conn->Execute($sql5))$depe_nom=$rs->fields['DEPE_NOMB'];$c=1;
$rse=$db->conn->Execute($sqld);
...
if($srd!="" or $sbrd!=""){
	include("$ruta_raiz/include/query/archivo/queryInventario.php");
if($rs=$db->conn->Execute($sqlsr))$srd_desc=$rs->fields['SGD_SRD_DESCRIP'];$c.=4;
if($rs=$db->conn->Execute($sqltsb)){
