...
<?php
	...
	while(!$rs->EOF){
		...
		if ($time<=$fechaIni and $fechaIni<=$time2){
				...
				$rsp=$db->conn->Execute($quer);
			}
		if ($fechaIni>=$time2){

				$rsp=$db->conn->Execute($quer2);
			}
		}
		}
		$rs->MoveNext();
