...
if($_GET['action']=='muni'){
    ?><select  class='select' id="mun" name="codMuni2" onchange="edi()"><option>-- Seleccione --</option><?php
	 echo       municipio($db,$_GET['depto']);
		  	?></select><?php
}

if($_GET['action']=='edificio'){
?><select class='select' id="exp_edificio2" name="exp_edificio2" onchange="piso('piso')"><option>-- Seleccione --</option><?php
          echo       edificio($db,$_GET['depto'],$_GET['muni'],$_GET['coddep']);
		  	?></select><?php
}
if($_GET['action']=='piso'){
    ?><select class='select' id="Piso" name="exp_piso2" onchange="wait('wait','Piso',1)"><option>-- Seleccione --</option><?php
...
if($_GET['action']=='wait'){
		if($_GET['pos']==NULL || $_GET['pos']==1){
		   ...
		   if($nombre[1]==Null){
		 //  echo $_GET['padre'];
?><input type="hidden" name="exp_entre" value="<?php echo $_GET['padre']; ?>">
<input type="hidden" name="exp_caja" value="<?php echo $_GET['padre']; ?>">
<?php
		   }
		   else{
	//	   echo $_GET['padre'];
			...
			<table width="100%" cellpadding="0" cellspacing="0"  >
				<tr>
			     <td width="25%" class='titulos2'><?php echo $nombre[1];?></td><td width="25%" class='titulos2'><select class='select' id="<?php echo $nombre[1];?>" name="<?php echo $nombre[1] ;?>" onchange="wait('<?php echo $nombre[1];?>Div','<?php echo $nombre[1];?>','2',<?php echo $_GET['con']+1;?>)"><option>-- Seleccione --</option><?php
		          echo      $nombre[0];
			  	?></select> </td><td   width="50%" class='titulos2'> 
			<div id="<?php echo $nombre[1] ;?>Div"></div></td>
  			</tr> </table>
			<div  id="waitDiv<?php echo $_GET['con']+1;?>"></div><?php
			}
		}
    else{
	   $nombre=wait($db,$_GET['padre']);
		  ...
		  // echo $_GET['padre'];
?><input type="hidden" name="exp_entre" value="<?php echo $_GET['padre']; ?>">
<input type="hidden" name="exp_caja" value="<?php echo $_GET['padre']; ?>"><?php
		   }
		   else{		   
	?> <table cellpadding="0" cellspacing="0"  width="100%"><TR><TD class='titulos2' width="50%"><?php echo $nombre[1]; ?></TD><TD width="50%"><select  id="<?php echo $nombre[1];?>" name="<?php echo $nombre[1] ;?>" class='select' onchange="wait('waitDiv<?php echo $_GET['con'];?>','<?php echo $nombre[1];?>',1,<?php echo $_GET['con']+1;?>)"><option>-- Seleccione --</option><?php
		          echo      $nombre[0];
...
/**
* @autor Hardy Deimont Nino Velasquez
* Funtion Ubicacion
