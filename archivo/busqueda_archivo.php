...
include_once("$ruta_raiz/include/db/ConnectionHandler.php");
	...
	$encabezadol = $_SERVER['PHP_SELF']."?".session_name()."=".session_id()."&dependencia=$dependencia&krd=$krd&sel=$sel";
...
<?php // parametrizacion de items
	...
	$item4=$tm[0];
	include("$ruta_raiz/include/query/archivo/queryBusqueda_exp.php");
	$rs=$db->conn->Execute($sql8);
	$item51=$rs->fields["SGD_EIT_NOMBRE"];
	$tm=explode('1',$item51);
	$item5=$tm[0];
	include("$ruta_raiz/include/query/archivo/queryBusqueda_exp.php");
	$rs=$db->conn->Execute($sql9);
...
<td width="20%" class='titulos2'>
          <?php
			...
			$rsPE=$db->conn->Execute($queryDpto);
 ...
 <td width="20%" class='titulos2'>
          <?php
          	$rsPX=$db->conn->Execute($queryMuni);
...
if(($buscar_docu!="" and $at=='000')){
 ...
 ?><tr>
	...
	if($valImg == "SI"){
		...
		?>
		<td class=leidos2 align="center"><a href='../verradicado.php?<?=$encabezado."&num_expediente=$expnum&verrad=$radi&carpeta_per=0&carpeta=8&nombcarpeta=Expedientes"?>' > <?php echo $fechrad;?></a></td>
	 <?php
	 } else {
