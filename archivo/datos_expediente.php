...
?>
  ...
  if($grabar=="No") $Archivar = "Nueva U";
      ...
      WHERE SGD_EXP_NUMERO LIKE '$num_expediente' and r.RADI_NUME_RADI=e.RADI_NUME_RADI AND e.SGD_EXP_ESTADO <> 2";
      
	    //if($exp_carpeta!="" and $car) $sqlrad3.=" and e.SGD_EXP_CARPETA LIKE '$carpe'";
	    $sqlrad3.=" ORDER BY e.SGD_EXP_CARPETA ,e.RADI_NUME_RADI";
    
	    $rsrad3=$db->conn->Execute($sqlrad3);
	    $j=1;
	    $exp_folio=0;
	    while(!$rsrad3->EOF){
...
// Aqui se accede a la clase class_control para actualizar expedientes.
  ...
  if($exp_caja){
    $iSql = " UPDATE SGD_EXP_EXPEDIENTE
     ...
     //$db->conn->debug = true;
     $resultadoGen = $db->conn->Execute($iSql);
     //$db->conn->debug = true;
     if($resultadoGen) {
...
//$objHistorico->insertarHistoricoExp($num_expediente,$arrayRad,$dep_sel ,$codusuario, $observa2, 57,1);
    ...
    $sql="select SGD_EIT_NOMBRE, SGD_EIT_CODIGO from SGD_EIT_ITEMS where SGD_EIT_CODIGO like '$exp_caja'";
  ...
  ?>
	 <TD class='titulos2'>
      ...
      $sql="select SGD_EIT_NOMBRE, SGD_EIT_CODIGO, SGD_EIT_COD_PADRE from SGD_EIT_ITEMS
              where SGD_EIT_COD_PADRE like '$itemCodigoPadre'
              $whereInicio
  ...
  $sql="select SGD_EIT_NOMBRE
	...
	$item5=explode('1',$item51);
	  ...
	  $noHojasXCarpeta=0;
	  while(!$rsrad->EOF){
		  // echo "Carpeta " . $rsrad->fields['SGD_EXP_CARPETA'] . "Rads " . $noHojasXCarpeta . "<br> ";
		$carpeta[$j]=$rsrad->fields['SGD_EXP_CARPETA'];
		if(!$carpeta[$j]) $carpeta[$j]=0;

		
    	if($noHojasXCarpeta>=205 and $carpetaOld!=$carpeta[$j] and $carpetaOld!=0){
		
	    ?>
	     <script language="javascript">
	      confirm('Debe hacer el cambio de carpeta '+<?=$carpetaOld?>+'. Numero de Folios:'+<?=$noHojasXCarpeta?>);
	     </script>
	    <?php 
		   $noHojasXCarpeta = 0; 
		}elseif($carpetaOld!=$carpeta[$j]){
			 $carpetaOld = $carpeta[$j];
			 $noHojasXCarpeta = 0; 
		}
		$radiNumeRadi=$rsrad->fields['RADI_NUME_RADI'];
        ...
        if($estiloCarpeta!=$nuevoEstiloCarpeta) {
	      ...
	      ?>   
                  ...
                  <?=$arrayRad[$t]?>
                   ...
                   <?php if ($esta[$t]=='10000' or $arrayRad[$t]==$nurad9)$st="checked"; else $st="";
                   if($fol[$t]=="")$fol[$t]=0;
                   ?>
