...
/**
* Env�a el formulario hacia el programa que realiza la edicion del grupo de radicados
*/
function enviar() {
  ...
  <table BORDER=0  cellpad=2 cellspacing='0' WIDTH=98% class='t_bordeGris' valign='top' align='center' >
     ...
     <td width='35%' >
      <table width='100%' border='0' cellspacing='1' cellpadding='0'>
		...
		<tr class="info">
          <td height="30"><?=$_SESSION['usua_nomb'] ?></td>
        </tr>
      </table>
    </td>
	 <td width='35%' >
      <table width='100%' border='0' cellspacing='1' cellpadding='0'>
		...
		<tr class="info">
          <td height="30"><?=$_SESSION['depe_nomb'] ?></td>
        </tr>
      </table>
    </td>
</TR>
      	...
      	<TD  >
           	...
           	<FORM name=form_busq_rad action='lista_sacar_grupo.php?<?=session_name()."=".trim(session_id())?>' method=post>
                Buscar radicado(s) (Separados por coma)<input name="busq_radicados" type="text" size="70" class="tex_area" value="<?=$busq_radicados?>">
							...
							<?//almacena los elementos de sesion
	 ...
	 <form method="post" action="sacar_grupo_registro.php?grupo=<?=$grupo?>&pagina=<?=$pagina?><?=session_name()."=".session_id()."&krd=$krd&fechah=$fechah&dep_sel=$dep_sel&estado_sal=$estado_sal&estado_sal_max=$estado_sal_max" ?>" name="formSacarGrupo">
		<input name="grupo" type="hidden" value="<?=$grupo?>" />
          ...
          <tr>
            <td class="grisCCCCCCs" valign="top" colspan="3" height="120">
                ...
                <?php
								  //var que recoge el numero de radicados del grupo
									$num = count($radsGrupo);
									$i = 0;
								  ...
								  $registro=$pagina*20;
									...
									while ($i < $num) {
									  ...
									  //Imprime solo los 20 de la pagina seleccionada
