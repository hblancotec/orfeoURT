...
function Header() {
    ...
    $this->SetXY($l,20+$margen);
 	$this->Cell(472,14,'REMITENTE: DEPARTAMENTO NACIONAL DE PLANECION',1,1,'L');
    $this->SetXY($l,34+$margen);
 	$this->Cell(472,13,'CALLE 26 # 13 - 9',1,1,'L');
    $this->SetXY($l,47+$margen);
 	$this->Cell(472,13,"CONTRATO $numContrato ",1,1,'L');
	$l = 500;
	$this->SetXY($l,20+$margen);
...
//        for($i=0;$i<$this->numFields-1;$i++){
        for($i=0;$i<$this->numFields-1;$i++){
	   $stringWidth = $this->getstringwidth($this->rs->FetchField($i)) + 6 ;
