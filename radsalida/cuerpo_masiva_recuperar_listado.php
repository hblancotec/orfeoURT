...
<tr>
	<td class="grisCCCCCC">
		...
		<tr bgcolor="#cccccc" class="textoOpcion">
			<td class="titulos3" width="10%">
				<a href='cuerpo_masiva_recuperar_listado.php?<?=$encabezado ?>1&ordcambio=1' class='textoOpcion' alt='Seleccione una busqueda'><?=$img1 ?>Grupo </a>
			</td>
			...
			<td width='7%' class="titulos3" align="center">
				<p>
					<a href='cuerpo_masiva_recuperar_listado.php?<?=$encabezado ?>4&ordcambio=1' class='textoOpcion' alt='Seleccione una busqueda'><?=$img4 ?>Fecha</a>
				</p>
			</td>
			<td width='10%' class="titulos3" align="center">Documentos</td>
			<td width='9%' class="titulos3" align="center">Eliminados del grupo</td>
			<td width='28%' class="titulos3" align="center">Generado <br> Por</td>
			<td width='12%' class="titulos3" align="center">Tipo<br></td>
			<td width='4%' class="titulos3" align="center">Enviar<br></td>
		</tr>
<?php
$i = 1;
$ki = 0;
...
?>
        	<td class='<?=$leido ?>' align="center" width="10%">
        		<a class="vinculos" href='lista_sacar_grupo.php?grupo=<?=$radi_nume_grupo?>&krd=<?=$krd?>'>
        			<span class="tgris"><?=$radi_nume_grupo ?></span>
        		</a>
        	</td>
			<td class='<?=$leido ?>' width="9%">
                    <?=$rs->fields['RAD_INI']; ?>
            </td>
			<td class='<?=$leido ?>' width="11%"><?=$rs->fields['RAD_FIN']; ?></td>
			<td class='<?=$leido ?>' width="7%"><?=$rs->fields['FECHA'];  ?></td>
			<td class='<?=$leido ?>' width="10%"><?=$documentos ?></td>
			<td class='<?=$leido ?>' width="9%"><?=$eliminados ?></td>
			<td class='<?=$leido ?>' width="28%"> &nbsp;<?=$usuarioGen->get_usua_nomb();?></td>
			<td class='<?=$leido ?>' width="12%"> <?=$tipoDoc->get_sgd_tpr_descrip() ?></td>
			<td align='center' class='<?=$leido ?>' width="4%"><INPUT type="radio" name="radGrupo" value="<?=$radi_nume_grupo ?>"></td>
		</tr>
<?php
        }
    $ki = $ki + 1;
...
$paginas = ($numerot / 20);
?>
	<span class='paginacion'> P&aacute;ginas</span> 
<?php
