<?php
    ...
    if (!$no_planilla or intval($no_planilla) == 0)
        die ("<p><hr><b><center><span class='etexto'>Debe colocar un Numero de Planilla v&aacute;lido");
    if($generar_listado) {
        $ruta_raiz = "..";
        ...
        $isql = "select * From sgd_enve_envioespecial where sgd_fenv_codigo = 110";
        ...
        include "$ruta_raiz/include/query/radsalida/queryPlanillaHoyMismo.php";	

        $whereV = " WHERE SGD_RENV_MPIO = '$depe_municipio' AND
                            DEPE_CODI = $dependencia AND
                            SGD_FENV_CODIGO = 110 AND
                            SGD_RENV_VALOR != 0 AND
                            $wplanilla AND
                            sgd_renv_tipo < 2";
    ...
    //		$rsUpValor->MoveFirst();
            ...
            for ($cont = 1; $cont <= $maximo; $cont++ ) {
                ...
                $rs = $db->conn->Execute($isql_updateLoc);	
                $rsUpValor->MoveNext();
            }
        }
        ...
        $whereV = " WHERE SGD_RENV_MPIO != '$depe_municipio' AND
                                ...
                                $wplanilla AND
                                sgd_renv_tipo <2";
        $sqlEnv = "select * from sgd_renv_regenvio  $whereV $sWhere $order_isql";
        $rsUpValor = $db->conn->Execute($sqlEnv);
        $nregis = 0 ;
        $rvCodigo = $rsUpValor->fields["SGD_RENV_CODIGO"] ;
        if ($rvCodigo)  {
    //		$rsUpValor->MoveFirst();
        ...
        do {  // Amplia
            ...
            if($generar_listado_existente)  {
               $i_total3 = 0;
                ...
                $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
                $sqlEnvio = "select * from sgd_renv_regenvio  $where_isql $order_isql";
                $rsParaUp = $db->conn->Execute($sqlEnvio);
    ...
    //				$rsParaUp->MoveFirst();
                ...
                if ($nregis > 0)  {
                    $sqlEnv2 = "select * from sgd_renv_regenvio  $where_isql $order_isql";
                    $rsParaUp = $db->conn->Execute($sqlEnv2);
    //				$rsParaUp->MoveFirst();	
                    if ($nregis <= 32) $maximo = $nregis; else $maximo = 32;
                    for ($cont = 1; $cont <= $maximo; $cont++ )  {
                        ...
                        $rs = $db->conn->Execute($update_isql);	
                        $rsParaUp->MoveNext();
                    }
                }
