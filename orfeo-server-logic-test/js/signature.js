...
function monitorWelcomeAction() {
    ...
    signMonitorLauncher.signerConfigurator.p12TabActive = true;

    if (globalTimestampFlag === "true")
        signMonitorLauncher.signerConfigurator.timestamp = tsa;
    flagRetry = true;
    hashProcess(globalTimeStamp, globalFile, globalUser, globalIdentification, globalTimestampFlag, globalVisible, globalLowerLeftX, globalLowerLeftY, globalUpperRightX, globalUpperRightY, globalQrCodeSize, globalQrCodeVisibility, globalPage, globalPlaceholder, qrPage, absoluteX, absoluteY, absoluteSize, renderingMode, description, qrBackgroundImageFlag, qrBackgroundImageOffset);
}
...
function postSign(signature, currentTimeStamp, currentFile, index, currentRadicado) {
    ...
    request.done(function (response) {
        if (response.indexOf("Firma digital generada con un certificado revocado") !== -1 && flagRetry === true) {
            flagRetry = false;
            hashProcess(globalTimeStamp, globalFile, globalUser, globalIdentification, globalTimestampFlag, globalVisible, globalLowerLeftX, globalLowerLeftY, globalUpperRightX, globalUpperRightY, globalQrCodeSize, globalQrCodeVisibility, globalPage, globalPlaceholder, qrPage, absoluteX, absoluteY, absoluteSize, renderingMode, description, qrBackgroundImageFlag, qrBackgroundImageOffset);
}
...
for (var j = 0; j < (arrayResponse.length - 1); j++) {
    ...
    if (element.indexOf("Firma digital generada exitosamente") === -1 || element.indexOf("Firma digital generada exitosamente") > 2) {
