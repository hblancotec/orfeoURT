...
switch ($evento) {
    ...
    case 3: // crea y modifica expediente
        ...
        $subSerie = str_pad($subSerie, 3, "0", STR_PAD_LEFT);
        $serie = str_pad($serie, 3, "0", STR_PAD_LEFT);
        $depenNum = str_pad($depen, 3, "0", STR_PAD_LEFT);   /// ojo para dependencia 4 digitos
        ...
        if ($nombExpExiste == 0) {
            ...
            if ($auto == 'false') {
                ...
                if (! preg_match('/' . "^[0-9]{18}[A-Z]{1}" . '/i', $numExpe)) {
                    salirError($mensaje9);
                    return;
                }
                /*
            ...
            $sqlRad = "SELECT ".$db->conn->SQLDate('Y-m-d H:i:s', 'R.RADI_FECH_RADI')." AS FECHA,
    ...
    case 5: // Incluir radicados en expediente
        {
            ...
            if ($_POST['codTdoc'] > 0) {
                ...
                $isqlTRD = "SELECT	SGD_MRD_CODIGO FROM SGD_MRD_MATRIRD WHERE DEPE_CODI = " . $_POST['depen'] . " AND SGD_SRD_CODIGO 	= " . $_POST['selectSerie'] . " AND SGD_SBRD_CODIGO = " . $_POST['selectSubSerie'] . " AND SGD_TPR_CODIGO 	= " . $_POST['codTdoc'];
    ...
    case 6: // Retornar Serie
        ...
        $sql1 = "	SELECT
							...
							--,RIGHT('000' + convert(varchar,US.USUA_CODI),3) AS CODIGO_USUARIO	
    ...
    case 8: // Retornar tipos documentales
             // si alguno de los siguientes parametros no esta, salga.
        if (empty($depeInput) || empty($serie) || empty($subSerie)) {
            salirError($mensaje0);
            return;
        }
